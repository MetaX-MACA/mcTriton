...
class Autotuner(KernelInterface):
    ...
    def _dump_cache(self, path, config):
        ...
        temp_path = f"{path}.tmp.pid_{pid}_{rnd_id}"
    ...
    def _parse_config_dict(self, config_dict):
        default_config_menbers = ("num_warps", "num_stages", "num_ctas", "maxnreg")
        kwargs = config_dict.copy()
        for menber in default_config_menbers:
            kwargs.pop(menber, None)
        maxnreg = config_dict["maxnreg"] if "maxnreg" in config_dict else None
        return Config(kwargs, config_dict["num_warps"], config_dict["num_stages"], config_dict["num_ctas"], maxnreg)
    ...
    def _config_cache_path(self, key):
        ...
        if user_defined_path is not None:
            base_path = user_defined_path
        src = inspect.getsource(self.base_fn)
        function_hash = hashlib.sha256(src.encode("utf-8")).hexdigest()
        cache_file_name = self.base_fn.__name__ + str(key) + ".json"
        cache_path = os.path.join(base_path, function_hash, cache_file_name)
        return cache_path

    def _enable_config_cache(self):
        return os.environ.get("TRITON_ENABLE_PERSISTENT_AUTOTUNE_CONFIGS", "0") == "1"
    ...
    def run(self, *args, **kwargs):
        ...
        if len(self.configs) > 1:
            ...
            if key not in self.cache:
                if self._enable_config_cache():
                    # load cache from persistent path
                ...
                if key not in self.cache:   
                    ...
                    if self._enable_config_cache():
                        self._dump_cache(self._config_cache_path(key), self.cache[key])
