...
def get_lld_version(lld_path='ld.lld'):
    try:
        result = subprocess.run([lld_path, '--version'], 
...
class CMakeBuild(build_ext):
    ...
    def build_extension(self, ext):
        ...
        if check_env_flag("TRITON_BUILD_DISTRIBUTED", "OFF"):  # Default OFF
        ...
        ld_lld_path = maca_path + "/mxgpu_llvm/bin/ld.lld"
        llvm_version = get_lld_version(ld_lld_path)
        if llvm_version == 19:  # Default ON
            cmake_args += ["-DTRITON_USE_OPAQUE_PTR=ON"]
            cmake_args += ["-DTRITON_USE_LLVM19_INTRINSIC=ON"]
        else:
            cmake_args += ["-DTRITON_USE_OPAQUE_PTR=OFF"]
            cmake_args += ["-DTRITON_USE_LLVM19_INTRINSIC=OFF"]
...
def add_link_to_tutorials():
    tutorials_dir = os.path.abspath(os.path.join(os.path.dirname(__file__), "tutorials"))
    tutorials_install_dir = os.path.join(os.path.dirname(__file__), "triton", "tutorials")
    if os.path.islink(tutorials_install_dir):
        os.unlink(tutorials_install_dir)
    if os.path.exists(tutorials_install_dir):
        shutil.rmtree(tutorials_install_dir)
    os.symlink(tutorials_dir, tutorials_install_dir)


def add_links():
    add_link_to_backends()
    add_link_to_tutorials()
    if check_env_flag("TRITON_BUILD_PROTON", "ON"):  # Default ON
        add_link_to_proton()
...
version = "3.0.0"
if os.getenv("TRITON_BUILD_MACA_VERSION"):
    version += "+metax" + os.getenv("TRITON_BUILD_MACA_VERSION")
