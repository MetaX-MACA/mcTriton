...
void init_triton_metax_passes_ttgpuir(py::module &&m) {
  ...
  m.def("add_to_llvmir", [](mlir::PassManager &pm, int32_t capability,
    ...
    pm.addPass(mlir::triton::METAX::createConvertTritonGPUToLLVMPass(
        capability, enSmIdxCache, enSmIndexOpt));
  });
  ...
  ADD_PASS_WRAPPER_4("add_pipeline_maca",
                     ...
                     bool);
  ADD_PASS_WRAPPER_0("add_prefetch_maca",
                     mlir::createTritonMETAXGPUPrefetchMACAPass);
  ADD_PASS_WRAPPER_0("add_tritonmetaxgpu_move_dot_operands_out_loop_pass",
                     mlir::createTritonMETAXGPUMoveDotOperandsOutLoopPass);
  ADD_PASS_WRAPPER_1("add_pipeline_async_tn",
                     mlir::createTritonMETAXGPUPipelineAsyncTNPass, int);
  ADD_PASS_WRAPPER_1("add_pipeline_async_tt",
                     mlir::createTritonMETAXGPUPipelineAsyncTTPass, int);
  ADD_PASS_WRAPPER_2("add_pipeline_async_base",
                     mlir::createTritonMETAXGPUPipelineAsyncBasePass, int,
                     bool);
  ADD_PASS_WRAPPER_0("add_tritonmetaxgpu_merge_equal_shared_layout_pass",
                     mlir::createTritonMETAXGPUMergeEqualSharedLayoutPass);
  ADD_PASS_WRAPPER_5(
      "add_pipeline_async_multidot_mla_mixed",
      mlir::createTritonMETAXGPUPipelineAsyncMultiDotMLAMixedPass, int, bool,
      int, bool, bool);
  ...
  ADD_PASS_WRAPPER_2("add_tritonmetaxgpu_change_layout_for_int8_pass",
                     ...
                     const std::string);
  ADD_PASS_WRAPPER_4("add_tritonmetaxgpu_change_convert_layout_attr",
                     mlir::createTritonMETAXGPUChangeConvertLayoutAttrPass,
                     bool, bool, bool, bool);
  ADD_PASS_WRAPPER_0(
      "add_tritonmetaxgpu_change_layout_for_constancy_load_layout",
      mlir::createTritonMETAXGPUChangeLayoutForConstancyLoadPass);
  ADD_PASS_WRAPPER_0("add_tritonmetaxgpu_merge_convert_layout_pass",
                     mlir::createTritonMETAXGPUMergeConvertLayoutPass);
}
...
void init_triton_metax(py::module &&m) {
  ...
  m.def(
      "mlir_opt",
      [](const std::string mlir_src, std::string mlir_opt,
         int llvm_version) -> py::object {
        ...
        if (llvm_version == 12) {
          mlir_opt_options += " --convert-to-llvm-12 ";
        }
  ...
  m.def("translate_mlir_to_llir",
        [](const std::string mlir_src, std::string maca_path) -> py::object {
          llvm::SmallString<64> fsrc;
          llvm::sys::fs::createTemporaryFile("compile-maca-src", "ll", fsrc);
          const char *_fsrc = fsrc.c_str();
          llvm::FileRemover srcRemover(fsrc);
          ...
          std::string mlir_translate =
              maca_path + "/mxgpu_llvm/bin/mlir-translate ";
          std::string cmd =
              mlir_translate +
              " -mlir-to-llvmir -split-input-file -verify-diagnostics " +
              _fsrc + " -o " + _fdst + " 2> " + _flog;
          if (std::getenv("TRITON_PRINT_COMPILE_OPTIONS")) {
            llvm::outs() << "translate_mlir_to_llir compile options:" << cmd
                         << "\n";
          }
          int err;
          err = system(cmd.c_str());
  ...
  m.def("translate_llvmir_to_mcfatbin",
        [](const std::string llvmIR, std::string mxcc_arch,
          ...
          cmd = mxcc_arch + dump_str + _fsrc + extra_option + opt_str +
                " -maca-link -input-is-device --maca-path " + maca_path +
                " -mllvm -inputFuncBC=" + maca_path + "/lib/maca_kernellib.bc" +
                " -o " + _fbin + " 2> " + _flog;
          if (std::getenv("TRITON_PRINT_COMPILE_OPTIONS")) {
            llvm::outs() << "compile options:" << cmd << "\n";
          }
          int err;
          err = system(cmd.c_str());
          if (err != 0) {
