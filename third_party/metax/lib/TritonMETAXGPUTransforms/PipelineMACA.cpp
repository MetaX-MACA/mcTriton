...
class LoopPipeliner {
  ...
  /// TODO(MACA): only allocated 2 buffers.
    ...
    if (auto loadOp = dyn_cast<triton::LoadOp>(op)) {
      ...
      if (isCandidate && loadOp.getResult().hasOneUse()) {
        ...
        while (use) {
          ...
          auto memType = dyn_cast<tt::MemDescType>(use->getResult(0).getType());
          if (tensorType &&
              !mlir::isa<ttg::SharedEncodingAttr>(tensorType.getEncoding()))
            break;
          if (memType &&
              !mlir::isa<ttg::SharedEncodingAttr>(memType.getEncoding()))
            break;
#ifdef USE_MACA
          dotDeps_.insert(use);
#endif
...
void LoopPipeliner::createBufferTypes() {
  for (auto loadCvt : loadsMapping) {
    ...
    if (auto transOp = llvm::dyn_cast<triton::TransOp>(trans)) {
      auto dstTy = dyn_cast<TensorOrMemDesc>(transOp.getSrc().getType());
      ...
      auto sharedEnc = ttg::SharedEncodingAttr::get(
          ty.getContext(), dotOpEnc, dstTy.getShape(),
          ttg::getOrder(dstTy.getEncoding()),
          ttg::getCTALayout(dstTy.getEncoding()), dstTy.getElementType(), true);
      loadsBufferType[loadOp] = mlir::triton::MemDescType::get(
          bufferShape, dstTy.getElementType(), sharedEnc, /*mutable*/ true);
    } else {
      auto dstTy = dyn_cast<TensorOrMemDesc>(cvtSrc.getType());
      SmallVector<int64_t> bufferShape(dstTy.getShape().begin(),
                                       dstTy.getShape().end());
      unsigned bitWidth = dstTy.getElementType().getIntOrFloatBitWidth();
      bufferShape.insert(bufferShape.begin(), numStagesForSHM);
      ...
      if (it != validCvts.end()) {
        ...
        if (mlir::isa<ttg::DotOperandEncodingAttr>(cvtDstTy.getEncoding())) {
          ...
          if (transOp) {
            ...
            if (cvtOp1) {
              ...
              auto cvt = builder.create<ttg::LocalLoadOp>(
                  transOp.getLoc(), cvtDstTy, newTrans, nullptr, true);
          ...
          auto cvt = builder.create<ttg::LocalLoadOp>(
              ...
              newForOp.getRegionIterArgs()[sliceIdx + loadArgIdx], nullptr,
              true);
          mapping.map(result, cvt.getResult());
          nextMapping.map(result, cvt.getResult());
          continue;
  ...
  for (Operation *op : orderedDeps) {
    ...
    if (validLoads.contains(op->getResult(0))) {
      ...
      newLoadOp = builder.create<triton::LoadOp>(
          ...
          loadOp.getBoundaryCheckAttr(), loadOp.getPaddingAttr(),
          loadOp.getCache(), loadOp.getEvict(), loadOp.getIsVolatile(), false);
