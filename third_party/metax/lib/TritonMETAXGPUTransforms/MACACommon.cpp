...
// Get the last power of 2 for an integer (or the integer itself if it is a
// power of 2).
// e.g. lastPowOf2(5)=4, lastPowOf2(8)=8
template <typename T> T lastPowOf2(T n) {
  if (n == 0) {
    return 0;
  }
  T m = 1;
...
void checkMaxElemsPerThread(const int &tileEdge,
                            const unsigned &mmaIntrinsicThreadEdge,
                            const int &warps, unsigned &maxElemsEdge) {
  while (tileEdge < mmaIntrinsicThreadEdge * maxElemsEdge * warps &&
         maxElemsEdge > 1) {
    maxElemsEdge /= 2;
  }
}

// always try to reture a squared shape
void spilt_numWarps(const int &numWarps, const int &maxWarpMA,
                    const int &maxWarpNA, const int &maxWarpMB,
                    const int &maxWarpNB, int &warpM, int &warpN) {
  int nw = log2(numWarps);
  int wm = 1, wn = 1;
  if (nw % 2 == 0) {
    wm = wn = nw / 2;
  } else {
    wm = ceil<unsigned>(nw, 2);
    wn = nw - wm;
  }
  warpM = 1 << wm;
  warpN = 1 << wn;

  if (maxWarpMA + maxWarpMB < maxWarpNA + maxWarpNB) {
    std::swap(warpM, warpN);
  }
}

// present we only check shared mem limit
// TODO(): maybe we can add register check
bool checkResourceLimit(const llvm::SmallVector<int, 4> &tile,
                        const llvm::SmallVector<unsigned, 3> &mmaIntrinsicSize,
                        const mlir::Type &dtype, const int &warpM,
                        const int &warpN,
                        const llvm::SmallVector<unsigned, 3> &elemsPerThread) {
  const int num_stages = 2;
  const int prefetch =
      std::getenv("TRITON_DISABLE_MACA_OPT_MMA_PREFETCH") == nullptr ? 2 : 1;

  const int shared_mem_limit = 64 * 1024; // 64kB
  const int dtype_bytes = dtype.getIntOrFloatBitWidth() / 8;
  int64_t CSize = 0;
  if (std::getenv("TRITON_DISABLE_CSTORE_OPT") != nullptr) {
    CSize = mmaIntrinsicSize[2] * elemsPerThread[2] * warpM *
            mmaIntrinsicSize[1] * elemsPerThread[1] * warpN * dtype_bytes;
  }
  if (CSize > shared_mem_limit) {
    // std::cout << "####shared mem check fail: " << CSize << std::endl;
    return false;
  }
  // std::cout << "####shared mem check pass: " << CSize << std::endl;
  return true;
}

} // namespace

llvm::SmallVector<unsigned, 3> getMMAIntrinsicThreadShape() {
  return {16, 16, 4};
}
...
bool updateLayout(llvm::SmallVector<unsigned, 3> &elemsPerThread,
  ...
  if (dtype.isF32() && !enableTf32) {
    ...
    if (chainDot) {
      isOpt = matchTable(pattern, fp16ChainTable, numWarps, enableTf32, dtype,
                         aorder, border, elemsPerThread, warpsPerTile, version);
    } else {
      if (computeCapability >= 86) {
        isOpt = matchTable(pattern, fp16table_86, numWarps, enableTf32, dtype,
                           ...
                           storeCoalesce, computeCapability);
      } else {
        isOpt =
    ...
    if (computeCapability >= 86) {
      isOpt =
          matchTable(pattern, i8table_86, numWarps, enableTf32, dtype, aorder,
                     border, elemsPerThread, warpsPerTile, storeCoalesce, computeCapability);
      auto tk = elemsPerThread[2];
      assert((tk >= 8 && tk % 8 == 0) &&
  ...
  } else if (dtype.isFloat8E5M2() || dtype.isFloat8E4M3FN()) {
...
bool matchTable(const std::pair<llvm::SmallVector<int, 4>, Layout> &pattern,
  ...
  } else { // calculate best config
    if (std::getenv("TRITON_ENABLE_CALCULATE_CONF") != nullptr) {
      calConfig(pattern.first, numWarps, enableTf32, dtype, aorder, border,
                elemsPerThread, warpsPerTile, computeCapability);
      return true;
    }
    return false;
  }
...
bool matchTable(const std::pair<llvm::SmallVector<int, 4>, Layout> &pattern,
                const TileTablePattern &table, const int &numWarps,
                const bool enableTf32, const mlir::Type dtype,
                const ArrayRef<unsigned> aorder,
                const ArrayRef<unsigned> border,
                llvm::SmallVector<unsigned, 3> &elemsPerThread,
                llvm::SmallVector<unsigned, 2> &warpsPerTile, int &version,
                bool storeCoalesce,
                int computeCapability) {
  if (table.count(pattern)) { // matche table
    auto newLayout = table.at(pattern)[0];
...
void calConfig(const llvm::SmallVector<int, 4> &tile, const int &numWarps,
               const bool enableTf32, const mlir::Type &dtype,
               const ArrayRef<unsigned> aorder, const ArrayRef<unsigned> border,
               llvm::SmallVector<unsigned, 3> &elemsPerThread,
               llvm::SmallVector<unsigned, 2> &warpsPerTile,
               int computeCapability) {
  auto mmaIntrinsicSize = getMMAIntrinsicThreadShape();
  unsigned maxElemsM = 1, maxElemsN = 1;
  unsigned maxElemsKA = 1, maxElemsKB = 1, maxElemsK = 1;
  int maxWarpMA = 1, maxWarpNA = 1, maxWarpMB = 1, maxWarpNB = 1;
  int warpM = 1, warpN = 1;
  unsigned dtype_bytes = dtype.getIntOrFloatBitWidth() / 8;

  if (aorder[1] == 1) {
    maxElemsM = std::max(
        unsigned(std::min(unsigned(ceil(tile[0] / mmaIntrinsicSize[0])),
                          unsigned(16 / dtype_bytes))),
        unsigned(1));
  }
  if (aorder[0] == 1) {
    maxElemsKA = std::max(
        unsigned(std::min(unsigned(ceil(tile[2], int(mmaIntrinsicSize[2]))),
                          unsigned(16 / dtype_bytes))),
        unsigned(1));
  }
  if (border[0] == 1) {
    maxElemsN = std::max(
        unsigned(std::min(unsigned(ceil(tile[1], int(mmaIntrinsicSize[1]))),
                          unsigned(16 / dtype_bytes))),
        unsigned(1));
  }
  if (border[1] == 1) {
  ...
  maxElemsK = std::max(maxElemsKA, maxElemsKB);

  maxWarpMA = std::max(
      lastPowOf2(std::min(ceil(tile[0], int(mmaIntrinsicSize[0] * maxElemsM)),
                          numWarps)),
      1);
  maxWarpNA = std::max(
      lastPowOf2(std::min(ceil(tile[2], int(mmaIntrinsicSize[2] * maxElemsK)),
                          numWarps)),
      1);
  maxWarpMB = maxWarpNA;
  maxWarpNB = std::max(
      lastPowOf2(std::min(ceil(tile[1], int(mmaIntrinsicSize[1] * maxElemsN)),
                          numWarps)),
      1);
  spilt_numWarps(numWarps, maxWarpMA, maxWarpNA, maxWarpMB, maxWarpNB, warpM,
                 warpN);

  // try to keep all warps work
  checkMaxElemsPerThread(tile[0], mmaIntrinsicSize[0], warpM, maxElemsM);
  checkMaxElemsPerThread(tile[2], mmaIntrinsicSize[2], warpN, maxElemsKA);
  checkMaxElemsPerThread(tile[2], mmaIntrinsicSize[2], warpM, maxElemsKB);
  checkMaxElemsPerThread(tile[1], mmaIntrinsicSize[1], warpN, maxElemsN);
  maxElemsK = std::max(maxElemsKA, maxElemsKB);

  llvm::SmallVector<unsigned, 3> elemsPerThread_ = {maxElemsM, maxElemsN,
                                                    maxElemsK};
  bool checkPass = true;
  while (!checkResourceLimit(tile, mmaIntrinsicSize, dtype, warpM, warpN,
                             elemsPerThread_) &&
         checkPass) {
    auto maxIt =
        std::max_element(elemsPerThread_.begin(), elemsPerThread_.end());
    if (*maxIt > 1) {
      *maxIt /= 2;
    } else {
      checkPass = false;
      // assert(0 && "Calculate config fail");
    }
  }

  auto defaultElemsPerThread = getDefaultElemsPerThread(dtype, enableTf32, computeCapability);
  if (elemsPerThread_[0] % defaultElemsPerThread[0] ||
      elemsPerThread_[1] % defaultElemsPerThread[1] ||
      elemsPerThread_[2] % defaultElemsPerThread[2]) {
    checkPass = false;
  }

  if (checkPass) {
    elemsPerThread.assign(elemsPerThread_.begin(), elemsPerThread_.end());
    warpsPerTile[0] = warpM;
    warpsPerTile[1] = warpN;
  }
}
...
getDefaultElemsPerThread(Type type, bool enableTf32, int computeCapability) {
  if (type.isF32() && enableTf32) {
    ...
    return {1, 1, 4};
  } else if (type.isFloat8E5M2() || type.isFloat8E4M3FN()) {
    assert(computeCapability >= 86);
    return {1, 1, 8};
  } else {
...
int getGVMNumberPerOp(BlockedEncodingAttr &enc, RankedTensorType &tensor) {
  ...
  auto tileShape = getShapePerCTATile(enc);
...
SmallVector<unsigned, 2> getOrder(triton::gpu::ConvertLayoutOp &op) {
  ...
  if (transOp) {
    ...
    if (auto transEnc =
            dyn_cast<SharedEncodingAttr>(transSrcTy.getEncoding())) {
    ...
    if (auto transSrcTy = dyn_cast<RankedTensorType>(transSrc.getType())) {
      if (auto transEnc =
        ...
        return {};
                   ...
                   dyn_cast<mlir::triton::MemDescType>(transSrc.getType())) {
      if (auto transEnc =
              dyn_cast<SharedEncodingAttr>(transSrcTy.getEncoding())) {
        Operation *cvtPre = transSrc.getDefiningOp();
        if (auto cvtPreOp = dyn_cast<triton::gpu::LocalAllocOp>(cvtPre)) {
...
bool isNeedMerge(SharedEncodingAttr srcSharedEnc,
                 SharedEncodingAttr dstSharedEnc, ArrayRef<int64_t> shape) {
...
calExtractTensorIdx(RankedTensorType tensorType, RankedTensorType subTensorType,
  ...
  if (isa<triton::gpu::MACAMmaEncodingAttr>(totalEnc)) {
    ...
    shapePerCTA = totalMMAEnc.getShapePerCTATile();
    for (int i = 0; i < 2; i++)
    ...
    subSizePerThread = subBlockEnc.getSizePerThread();
    totalSizePerThread = totalBlockEnc.getSizePerThread();
    shapePerCTA = totalBlockEnc.getShapePerCTATile();
    for (int i = 0; i < 2; i++)
      numCTAs.push_back(totalTensorSize[i] / shapePerCTA[i]);
    order = mlir::triton::gpu::getOrder(totalBlockEnc);
    rank = order.size();
    ...
    subSizePerThread = subDotEnc.getSizePerThread();
    totalSizePerThread = totalDotEnc.getSizePerThread();
    shapePerCTA = totalDotEnc.getShapePerCTATile();
    for (int i = 0; i < 2; i++)
      numCTAs.push_back(totalTensorSize[i] / shapePerCTA[i]);
...
void genSwiMask(Value dep, unsigned inVec,
                ...
                triton::gpu::SharedEncodingAttr resEncoding, OpBuilder &builder,
                ArrayRef<int64_t> shape, ArrayRef<int64_t> tileIdx,
                int numStages, ArrayRef<int64_t> elemTileNum,
                IRMapping &mapping, Location loc) {
  ...
  if (auto cmpIOp = dyn_cast<arith::CmpIOp>(depOp)) {
    ...
    if (lhsTy.getRank() == 1) {
      ...
      if (shape.size() == 2 && product<int64_t>(elemTileNum) > 1 &&
        ...
        auto sizePerThread = newEnc.getSizePerThread();
        for (int i = 0; i < sizePerThread.size(); i++) {
          sizePerThread[i] /= elemTileNum[i];
        }
...
Value genSwiSubMask(Value mask, triton::LoadOp &op,
                    triton::gpu::BlockedEncodingAttr srcEncoding,
                    triton::gpu::SharedEncodingAttr resEncoding,
                    OpBuilder &builder, ArrayRef<int64_t> shape,
                    ArrayRef<int64_t> tileIdx, int numStages,
                    ArrayRef<int64_t> elemTileNum) {
  ModuleOp moduleOp = mask.getDefiningOp()->getParentOfType<ModuleOp>();
  triton::ModuleAxisInfoAnalysis axisInfoAnalysis(moduleOp);
  unsigned inVec = axisInfoAnalysis.getPtrContiguity(op.getPtr());
  IRMapping maskDepsMapping;
  SmallVector<Operation *> orderedMaskDeps;
  bool validMaskSwizzle =
      checkMaskDepsRecursion(mask, inVec, axisInfoAnalysis, orderedMaskDeps,
                             resEncoding) > 1;
  assert(validMaskSwizzle);
  for (auto dep : orderedMaskDeps) {
    genSwiMask(dep->getResult(0), inVec, axisInfoAnalysis, srcEncoding,
               resEncoding, builder, shape, tileIdx, numStages, elemTileNum,
               maskDepsMapping, op.getLoc());
  }
...
bool checkUseCpAsync(triton::LoadOp &op, mlir::Type elemTy,
  ...
  auto srcBlocked =
      dyn_cast<triton::gpu::BlockedEncodingAttr>(srcTy.getEncoding());
  auto inOrder = getOrder(srcBlocked);
  ...
  auto sizePerThread = triton::gpu::getSizePerThread(srcBlocked);
...
checkMaskDeps(scf::ForOp &forOp, SetVector<Operation *> &ops,
  ...
  for (Operation *op : ops) {
    if (auto loadOp = dyn_cast<triton::LoadOp>(op)) {
      auto resSharedLayout = dyn_cast<triton::gpu::SharedEncodingAttr>(
      ...
      if (auto mask = loadOp.getMask()) {
        SmallVector<Operation *> deps;
        unsigned inVec = axisInfoAnalysis.getPtrContiguity(loadOp.getPtr());
        float mask_signal = checkMaskDepsRecursion(
            mask, inVec, axisInfoAnalysis, deps, resSharedLayout);
...
void checkMatchCpAsync(scf::ForOp &forOp, SetVector<Operation *> &ops,
                       DenseMap<Value, bool> &loadGenMask,
                       DenseMap<Value, triton::MemDescType> &loadsBufferType,
...
collectValidLoadRecursion(scf::ForOp &forOp, scf::YieldOp &yieldOp,
  ...
  if (auto loadOp = dyn_cast<triton::LoadOp>(op)) {
    ...
    auto srcTy = isa<triton::MemDescType>(transSrc.getType());
    ...
    if (v.getParentRegion() != forOp.getRegion())
      if (auto ty = dyn_cast<triton::MemDescType>(v.getType()))
        return std::make_pair(SetVector<Operation *>(),
                              SetVector<Operation *>());
    SetVector<Operation *> loads;
    SetVector<Operation *> deps;
    ...
    for (Value operand : op->getOperands()) {
      auto operandTy = llvm::dyn_cast<TensorOrMemDesc>(operand.getType());
      if (!operandTy)
        return std::make_pair(SetVector<Operation *>(),
                              SetVector<Operation *>());
...
LogicalResult collectValidOpMultiDot(
    scf::ForOp &forOp, scf::YieldOp &yieldOp, int numStages,
    DenseMap<Value, Value> &loadsMapping, DenseMap<Value, Value> &elemsMapping,
    SetVector<Value> &validLoads,
    DenseMap<Value, SetVector<Operation *>> &dotsDeps,
    DenseMap<Value, SetVector<Operation *>> &dotsDepsOutOfLoop,
    SetVector<Operation *> &ops, DenseMap<Value, Value> &loadDotMapping,
    bool enableDotStage) {
  // collect dots & loads.
  SetVector<triton::DotOp> dotsInLoop;
  for (Operation &op : forOp) {
  ...
  if (enableDotStage && dotsInLoop.size() < 1)
    return failure();
  // collect valid loadOp.
  // for multi dot, we need to collect a and b separatly
  for (triton::DotOp dot : dotsInLoop) {
    SetVector<Operation *> aDeps;
    SetVector<Operation *> bDeps;
    DenseMap<Value, Value> cvtMapping;
    // TODO(MACA):
    // collect operands of a
    SetVector<Value> aLoad;
    SetVector<Value> bLoad;
    auto aOps = collectValidLoadRecursion(forOp, yieldOp, cvtMapping, numStages,
                                          dot.getA(), ops);
    // collect operands of b
    auto bOps = collectValidLoadRecursion(forOp, yieldOp, cvtMapping, numStages,
    ...
    for (Operation *op : aOps.first) {
      ...
      if (isa<triton::LoadOp>(op)) {
        ...
        validLoads.insert(res);
        dotsDeps[res] = {};
        dotsDepsOutOfLoop[res] = {};
        ops.insert(op);
        // aLoad = res;
        aLoad.insert(res);
        auto loadOp = dyn_cast<triton::LoadOp>(op);
        loadDotMapping[loadOp] = dot;
      } else {
        elemsMapping[res] = cvtMapping[res];
      }
    }
    ...
    aDeps.insert(dot);
    bDeps.insert(dot);
    // TODO: maybe check aDeps intersect with bDeps?
    removeReplicateOp(aDeps);
    if (aLoad.size() > 0) {
      for (auto a : aLoad)
        reorderedDeps(forOp, dotsDeps[a], dotsDepsOutOfLoop[a], aDeps);
    }
    // reorderedDeps(forOp, dotsDeps[aLoad], dotsDepsOutOfLoop[aLoad], aDeps);
    removeReplicateOp(bDeps);
    if (bLoad.size() > 0) {
      for (auto b : bLoad)
        reorderedDeps(forOp, dotsDeps[b], dotsDepsOutOfLoop[b], bDeps);
    }
    // reorderedDeps(forOp, dotsDeps[bLoad], dotsDepsOutOfLoop[bLoad], bDeps);
  }
  if (validLoads.empty())
    return failure();
  return success();
...
removeReplicateOps(SmallVector<Operation *> genDeps) {
  ...
  for (Operation *op : genDeps) {
    ...
    if (auto cvt = dyn_cast<triton::gpu::ConvertLayoutOp>(op)) {
      ...
      auto srcTy = dyn_cast<TensorOrMemDesc>(src.getType());
      auto resTy = dyn_cast<TensorOrMemDesc>(res.getType());
      ...
      auto transTy = dyn_cast<triton::MemDescType>(src.getType());
      Attribute srcAttr = transTy.getEncoding();
      auto transResTy = dyn_cast<triton::MemDescType>(res.getType());
      ...
      if (srcAttr == resAttr) {
        ...
        auto srcOrder = triton::gpu::getOrder(srcAttr);
        auto dstOrder = triton::gpu::getOrder(resAttr);
    ...
    if (auto trans = dyn_cast<triton::TransOp>(op)) {
      ...
      if (auto slice = dyn_cast<triton::gpu::SliceEncodingAttr>(resAttr)) {
        newAttr = triton::gpu::SliceEncodingAttr::get(
            resTy.getContext(), slice.getDim(), cvtDstAttr);
      } else {
        newAttr = cvtDstAttr;
      }
      ...
      auto resShared = dyn_cast<triton::gpu::SharedEncodingAttr>(resAttr);
      ...
      for (auto v : op->getOperands()) {
        ...
        assert(operandVal);
        auto operandTy = cast<TensorOrMemDesc>(operandVal.getType());
        Attribute operandAttr = operandTy.getEncoding();
        auto operandShared =
            dyn_cast<triton::gpu::SharedEncodingAttr>(operandAttr);
        auto operandDot =
        ...
        if (operandShared) {
          ...
          if (transOp) {
            auto operandOrder = triton::gpu::getOrder(operandAttr);
            auto transSrc = transOp.getSrc();
            auto transTy = dyn_cast<triton::MemDescType>(transSrc.getType());
            assert(transTy);
            auto transSrcOrder = triton::gpu::getOrder(transTy.getEncoding());
          ...
          Value lload = builder.create<triton::gpu::LocalLoadOp>(
              op->getLoc(), newDstTy, newOperand, nullptr, true);
          if (enableDotStage) {
          ...
          if (resShared || resDot) {
            ...
            triton::MemDescType allocTy =
                cast<triton::MemDescType>(alloc.getType());
            Value zero =
                builder.create<arith::ConstantIntOp>(forOp.getLoc(), 0, 32);
            genDeps.push_back(zero.getDefiningOp());
            SmallVector<Value> copyOffsets(allocTy.getRank(), zero);
            copyOffsets[0] = extractSliceIdx;
            triton::MemDescType subviewTy = triton::MemDescType::get(
                allocTy.getShape().drop_front(), allocTy.getElementType(),
                allocTy.getEncoding(), /*mutableMemory=*/true);
            auto loc = op->getLoc();
            auto view = builder.create<triton::gpu::MemDescSubviewOp>(
                loc, subviewTy, alloc, copyOffsets);
...
bool checkPipelineLoadNumLegal(SetVector<Operation *> &ops,
                               SmallVector<Operation *> &orderedDeps,
                               DenseMap<Value, Value> &loadsMapping,
                               int pipelineLoadNum) {
  if (pipelineLoadNum == -1 || pipelineLoadNum > ops.size())
    return true;
  SetVector<Operation *> handledOps;
  for (auto it = ops.rbegin(); it != ops.rend(); ++it) {
    auto op = *it;
    if (handledOps.count(op) > 0)
      continue;
    int sharedCvtCnt = 1;
    bool ordered = true;
    handledOps.insert(op);
    for (auto [load, cvt] : loadsMapping) {
      if (loadsMapping[op->getResult(0)] == cvt && load.getDefiningOp() != op) {
        if (ops.count(load.getDefiningOp()) == 0)
          return false;
      }
    }
    for (auto itt = ops.rbegin(); itt != ops.rend(); ++itt) {
      auto opt = *itt;
      if (handledOps.count(opt) > 0)
        continue;
      if (loadsMapping[op->getResult(0)] == loadsMapping[opt->getResult(0)]) {
        if (!ordered)
          return false;
        sharedCvtCnt++;
        handledOps.insert(opt);
      } else {
        ordered = false;
      }
    }
    pipelineLoadNum -= sharedCvtCnt;
    if (pipelineLoadNum == 0) {
      return true;
    } else if (pipelineLoadNum < 0) {
      return false;
    }
  }
  return true;
...
mlir::Attribute getDotOperandsAttr(Value v) {
  ...
  while (op) {
    ...
    if (auto cvt = dyn_cast<triton::gpu::LocalLoadOp>(op)) {
      ...
      if (auto memTy = dyn_cast<triton::MemDescType>(cvtTy)) {
        if (mlir::isa<triton::gpu::SharedEncodingAttr>(memTy.getEncoding())) {
