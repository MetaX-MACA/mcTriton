...
private:
  void replaceLoadWithLayout(tt::LoadOp targetLoad,
    ...
    auto newLoadOp = builder.create<triton::LoadOp>(
        ...
        targetLoad.getIsVolatile(), targetLoad.getIsPipeline(),
        targetLoad.getIntrinsic(), targetLoad.getContiguityInterConstGroup());
  ...
  int getDivContiguityInterConstGroup(
    ...
    if (auto broadcastOp = dyn_cast<tt::BroadcastOp>(op)) {
      ...
      if (!srcTy)
        return -1;
      auto srcOrder = ttg::getOrder(srcTy.getEncoding());
