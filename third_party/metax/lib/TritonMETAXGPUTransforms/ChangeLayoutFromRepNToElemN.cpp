...
public:
  ...
  void runOnOperation() override {
    ...
    getOperation()->walk([&](scf::ForOp forOp) -> void {
      ...
      auto sizePerThread = originBlockedLayout.getSizePerThread();
      ...
      auto originShapePerCTA = originBlockedLayout.getShapePerCTATile();

      auto originRepN =
          ceil<unsigned>(resShape[order[1]], originShapePerCTA[order[1]]);
      auto totalElemsPerThreadN = originBlockedLayout.getElemsPerThread(
          resShape, tensorType.getElementType())[order[1]];
