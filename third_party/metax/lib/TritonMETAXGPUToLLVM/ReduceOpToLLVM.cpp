...
private:
  ...
  createPkFmaSumOp(triton::ReduceOp op, Location loc,
    ...
    for (int i = 0; i < operands[0].size(); i++) {
      ...
      vec0 = insert_element(vec0, operands[0][i], i32_val(0));
      vec0 = insert_element(vec0, operands[1][i], i32_val(1));
      vec1 = insert_element(vec1, one, i32_val(0));
      vec1 = insert_element(vec1, one, i32_val(1));
      vec2 = insert_element(vec2, operands[2][i], i32_val(0));
      vec2 = insert_element(vec2, operands[3][i], i32_val(1));
      ...
      results[0] = {extract_element(ret, i32_val(0))};
      results[1] = {extract_element(ret, i32_val(1))};
    }
    return results;
  }
  ...
  void sum_opt_using_pk_fma(
    ...
    for (const auto &[_, i] : uniqueOffsets) {
      ...
      accumulate(rewriter, *combineOp, accs[key], pk_fma_rets2[0], isFirst);
      accumulate(rewriter, *combineOp, accs[key], pk_fma_rets2[1], false);
    }
  }
...
#ifdef USE_MACA
        shfl[i] =
            targetInfo.shuffleXor(rewriter, loc, acc[i], N * interleave, op);
#else
  ...
  void storeWarpReduceToSharedMemory(
    ...
    if (auto mmaLayout = mlir::dyn_cast<MACAMmaEncodingAttr>(srcLayout)) {
    ...
    for (auto it : accs) {
      ...
      for (unsigned i = 0; i < op.getNumOperands(); ++i) {
        ...
        Value writePtr = gep(ptr_ty(rewriter.getContext(), elemTy, 3), elemTy,
  ...
  void accumulatePartialReductions(ReduceOpHelper &helper,
    ...
    for (unsigned round = 0; round < elemsPerThread; ++round) {
      ...
      for (unsigned i = 0; i < op.getNumOperands(); ++i) {
        auto elemTy = getElementType(op, i);
#ifndef USE_MACA_OPAQUE_PTR
        Value readPtr = gep(ptr_ty(rewriter.getContext(), elemTy, 3), elemTy,
      ...
      for (unsigned i = 0; i < op.getNumOperands(); ++i) {
        auto elemTy = getElementType(op, i);
#ifndef USE_MACA_OPAQUE_PTR
        writePtrs[i] = gep(ptr_ty(rewriter.getContext(), elemTy, 3), elemTy,
                           smemBases[i], writeOffset);
#else
