...
// TODO(liuyuxin): Support mma instructions.
...
static std::tuple<int, int> getRepMN(const RankedTensorType &tensorTy) {
  auto mmaLayout = cast<MmaEncodingAttr>(tensorTy.getEncoding());
...
Value loadC(Value tensor, Value llTensor,
  ...
  assert(isa<MmaEncodingAttr>(tensorTy.getEncoding()) &&
...
LogicalResult convertDot(const LLVMTypeConverter *typeConverter,
  ...
  auto mmaLayout = cast<MmaEncodingAttr>(cTensorTy.getEncoding());
  ...
  auto callF32Mma = [&](unsigned m, unsigned n, unsigned k, unsigned a,
...
#ifdef USE_BUILTIN
    // TODO(yaxing.feng@metax-tech.com): remove this if we support
    // ::mxgpu::mma** etc in maca
    ...
    if (mma_instr == "mma_16x16x16bf16") {
      ...
      auto convertI16ToHalf = [&](Value a) -> Value {
        ...
        Value v4_va = undef(v4_ta);
        for (int i = 0; i < 4; ++i) {
          Value ta_value = extract_element(
              IntegerType::get(rewriter.getContext(), 16), a, i32_val(i));
          Value cast_ta_value =
              bitcast(ta_value, FloatType::getF16(rewriter.getContext()));
          v4_va = insert_element(v4_ta, v4_va, cast_ta_value, i32_val(i));
        }
        return v4_va;
      };
    ...
    for (int i = 0; i < 4; ++i) {
      Value v1_value = extract_element(f32_ty, v4_value, i32_val(i));
    ...
    // auto aArgs = builder.newListOperand({{ha[{m, k}], "r"}});
    // auto bArgs = builder.newListOperand({{hb[{n, k}], "r"}});
    // auto cArgs = builder.newListOperand();
    // for (int i = 0; i < 4; ++i) {
    //   // fc size is elementPerThread * numRepN * numRepM
    //   cArgs->listAppend(builder.newOperand(fc[m * colsPerThread +
    //   elementPerThread * n + i],
    //                                        std::to_string(i)));
    //   // reuse the output registers
    // }

    // mma(retArgs, aArgs, bArgs, cArgs);
    // Value mmaOut = builder.launch(rewriter, loc, getMmaRetType(mmaType,
    // ctx));

    SmallVector<Type> retTys(4, FloatType::getF32(ctx));
    Type i128_t = IntegerType::get(ctx, 128);
    auto v4_t = vec_ty(FloatType::getF32(ctx), 4);
    llvm::SmallVector<Value, 3> args_value;
    args_value.push_back(ha[{m * elemsM + a, k}]); // a
    args_value.push_back(hb[{n * elemsN + b, k}]); // b
    Value v4_v = undef(v4_t);
    for (size_t ii = 0; ii < 4; ++ii) {
      v4_v = insert_element(
          v4_t, v4_v,
          fc[(m * colsPerThread + elementPerThread * n + ii) * elemsM * elemsN +
             a * elemsN + b],
          LLVM::createIndexConstant(rewriter, loc, typeConverter, ii));
    }

    Value i128c = bitcast(v4_v, i128_t);
    args_value.push_back(i128c); // c
    ArrayRef<Attribute> attrs = {};
    auto inlineAsm = rewriter.create<LLVM::InlineAsmOp>(
        loc, IntegerType::get(ctx, 128), args_value, mma_instr, "=r,r,r,r",
        true, /* hasSideEffect */ false, /* isAlignStack */
        LLVM::AsmDialectAttr::get(ctx, LLVM::AsmDialect::AD_ATT),
        ArrayAttr::get(ctx, attrs));
    Value v4_value =
        bitcast(inlineAsm.getRes(),
                LLVM::getFixedVectorType(IntegerType::get(ctx, 32), 4));

    for (int i = 0; i < 4; ++i) {
      Value v1_value = extract_element(
          IntegerType::get(ctx, 32), v4_value,
          LLVM::createIndexConstant(rewriter, loc, typeConverter, i));
      fc[(m * colsPerThread + elementPerThread * n + i) * elemsM * elemsN +
         a * elemsN + b] = bitcast(v1_value, FloatType::getF32(ctx));
  ...
  auto callTF32Mma = [&](unsigned m, unsigned n, unsigned k, unsigned a,
    ...
    Value aElemX2 = undef(fp32x2Ty);
    for (size_t i = 0; i < 2; i++) {
      aElemX2 = insert_element(fp32x2Ty, aElemX2,
                               ha[{m * elemsM + a, k * elemsK + c * 2 + i}],
                               i32_val(i));
    }
    Value bElemX2 = undef(fp32x2Ty);
    for (size_t i = 0; i < 2; i++) {
      bElemX2 = insert_element(fp32x2Ty, bElemX2,
                               hb[{n * elemsN + b, k * elemsK + c * 2 + i}],
                               i32_val(i));
    }
...
#else
    assert(false && "USE_BUILTIN is not defined.");
#endif
  };
  ...
  auto callF16Mma = [&](unsigned m, unsigned n, unsigned k, unsigned a,
    ...
    Value a4_v = undef(a4_t);
    Value b4_v = undef(b4_t);
    for (size_t ii = 0; ii < operandElemsPerMMA; ++ii) {
      ...
      if (inElemTy.isBF16()) {
        a_v = bitcast(a_v, f16_ty);
        b_v = bitcast(b_v, f16_ty);
      }
      a4_v = insert_element(a4_t, a4_v, a_v, i32_val(ii));
      b4_v = insert_element(b4_t, b4_v, b_v, i32_val(ii));
    }
    llvm::SmallVector<Value, 3> args_value;
    ...
    for (size_t ii = 0; ii < retElemsPerMMA; ++ii) {
      if (mmaLayout.getColMajor()) {
        if (mmaLayout.getIsBTrans() || mmaLayout.getIsATrans()) {
          v4_v = insert_element(
              v4_t, v4_v,
              fc[m * colsPerThread * elemsM * elemsN +
                 retElemsPerMMA * n * elemsN * elemsM + ii +
                 a * elemsN * retElemsPerMMA + b * retElemsPerMMA],
              i32_val(ii));
        } else {
          v4_v =
              insert_element(v4_t, v4_v,
                             fc[m * colsPerThread * elemsM * elemsN +
                                retElemsPerMMA * n * elemsN * elemsM +
                                ii * elemsN + a * elemsN * retElemsPerMMA + b],
                             i32_val(ii));
        }
      } else {
        v4_v = insert_element(
    ...
    for (int i = 0; i < retElemsPerMMA; ++i) {
      ...
      if (mmaLayout.getColMajor()) {
        if (mmaLayout.getIsBTrans() || mmaLayout.getIsATrans()) {
          fc[m * colsPerThread * elemsM * elemsN +
             retElemsPerMMA * n * elemsN * elemsM + i +
             a * elemsN * retElemsPerMMA + b * retElemsPerMMA] = v1_value;
        } else {
          fc[m * colsPerThread * elemsM * elemsN +
  ...
  auto callF64Mma = [&](unsigned m, unsigned n, unsigned k) {
    ...
    auto mma_instr = getMmaInstr(mmaType).str();
#ifdef USE_BUILTIN
    auto v4_t = vec_ty(FloatType::getF64(ctx), 4);
    llvm::SmallVector<Value, 3> args_value;
    args_value.push_back(ha[{m, k}]); // a
    args_value.push_back(hb[{n, k}]); // b
    Value v4_v = undef(v4_t);
    for (size_t ii = 0; ii < 4; ++ii) {
      v4_v = insert_element(v4_t, v4_v,
                            fc[m * colsPerThread + elementPerThread * n + ii],
                            i32_val(ii));
    }
    args_value.push_back(v4_v); // c
    ValueRange valueRange(args_value);
    ...
    for (int i = 0; i < 4; ++i) {
      Value v1_value =
          extract_element(FloatType::getF64(ctx), v4_value, i32_val(i));
      fc[m * colsPerThread + elementPerThread * n + i] = v1_value;
    }
#else
    mma_instr += " $0, $1, $2, $3";

    SmallVector<Type> retTys(4, FloatType::getF64(ctx));
    auto v4_t = vec_ty(FloatType::getF64(ctx), 4);
    llvm::SmallVector<Value, 3> args_value;
    args_value.push_back(ha[{m, k}]); // a
    args_value.push_back(hb[{n, k}]); // b
    ...
    args_value.push_back(v4_v);
    ArrayRef<Attribute> attrs = {};
    auto inlineAsm = rewriter.create<LLVM::InlineAsmOp>(
        loc, vec_ty(FloatType::getF64(ctx), 4), args_value, mma_instr,
        "=r,r,r,r", true, /* hasSideEffect */ false, /* isAlignStack */
        LLVM::AsmDialectAttr::get(ctx, LLVM::AsmDialect::AD_ATT),
        ArrayAttr::get(ctx, attrs));
    Value v4_value = inlineAsm.getRes();

    for (int i = 0; i < 4; ++i) {
      Value v1_value =
          extract_element(FloatType::getF64(ctx), v4_value, i32_val(i));
      fc[m * colsPerThread + elementPerThread * n + i] =
          bitcast(v1_value, FloatType::getF64(ctx));
    }
#endif
  };
  ...
  auto callB8Mma = [&](unsigned m, unsigned n, unsigned k, unsigned a,
    ...
    if (operandElemsPerMMA / 4 == 1) {
      cast_ha_value = bitcast(a4_v, i32_ty);
      cast_hb_value = bitcast(b4_v, i32_ty);
    } else {
      Type operandType = vec_ty(i32_ty, operandElemsPerMMA / 4);
      cast_ha_value = bitcast(a4_v, operandType);
      cast_hb_value = bitcast(b4_v, operandType);
    }
    ...
    Value v4_value = createBuiltinFunc<DotOp>(rewriter, loc, op,
                                              ...
                                              v4_v.getType(), valueRange);
    for (int i = 0; i < retElemsPerMMA; ++i) {
      Value v1_value = extract_element(resTy, v4_value, i32_val(i));
      fc[(m * colsPerThread + retElemsPerMMA * n + i) * elemsM * elemsN +
         a * elemsN + b] = v1_value;
    }
  };
          ...
          for (int b = 0; b < elemsN; ++b)
            for (int c = 0; c < elemsK_; ++c)
              if (resElemTy.isF32()) {
                ...
                } else if (inElemTy.isFloat8E5M2() ||
                           inElemTy.isFloat8E4M3FN()) {
...
LogicalResult convertMMAMACA(triton::DotOp op, triton::DotOp::Adaptor adaptor,
                             ...
                             ConversionPatternRewriter &rewriter) {
  auto loc = op.getLoc();
  auto mmaLayout = cast<MmaEncodingAttr>(
