...
static Value Fp8E4M3FN_to_Fp32_oneValue(Location loc,
  ...
  a = insert_element(fp8x4VecTy, a, int_val(8, 0), i32_val(1));
  a = insert_element(fp8x4VecTy, a, int_val(8, 0), i32_val(2));
  a = insert_element(fp8x4VecTy, a, v, i32_val(3));
  a = bitcast(a, i32_ty);

  // Get sign and absolute value
  Value sign = and_(a, int_val(32, 0x8000));
  a = and_(a, int_val(32, 0x7FFFFFFFF));

  // Right shift 1 bit to adjust the positions of exponent and mantissa
  a = lshr(a, int_val(32, 4));

  // Adjust exponent, (127 - 7) << 23 === 0xF000000
  a = add(a, int_val(32, 0xF000000));

  // Check NaN
  Value vAbs = and_(bitcast(v, i8_ty), int_val(8, 0x7F));
  a = select(icmp_eq(vAbs, int_val(8, 0x7F)), int_val(32, 0x7E00), a);
  ...
  for (int i = 0; i < lutSize; i++) {
    a = select(icmp_eq(vAbs, int_val(8, i)), int_val(32, denormsAndZeroLut[i]),
               a);
  }
...
static Value Fp8E4M3FN_to_Fp16_oneValue(Location loc,
  ...
  // Get sign and absolute value
  Value sign = and_(a, int_val(16, 0x8000));
  a = and_(a, int_val(16, 0x7FFF));

  Value vAbs = and_(v_16, int_val(16, 0x7FFF));

  // Compute renorm shift
  Value renorm_shift = int_val(16, 0);
  renorm_shift =
      select(icmp_ule(vAbs, int_val(16, 0x0700)), int_val(16, 1), renorm_shift);
  renorm_shift =
      select(icmp_ule(vAbs, int_val(16, 0x0300)), int_val(16, 2), renorm_shift);
  renorm_shift =
      select(icmp_eq(vAbs, int_val(16, 0x0100)), int_val(16, 3), renorm_shift);

  // Right shift 1 bit to adjust the positions of exponent and mantissa
  a = shl(a, renorm_shift);
  a = lshr(a, int_val(16, 1));

  // Adjust exponent compensator, (15 - 7 - renorm_shift) << 10
  Value exponent_compensator =
      shl(sub(int_val(16, 8), renorm_shift), int_val(16, 10));
  a = add(a, exponent_compensator);

  // Check NaN
  a = select(icmp_eq(vAbs, int_val(16, 0x7F00)), int_val(16, 0x7E00), a);

  // Check Zero
  a = select(icmp_eq(vAbs, int_val(16, 0x0000)), int_val(16, 0x0000), a);
...
struct FpToFpOpConversion
  ...
  convertFp32ToBf16(Location loc, T op, ConversionPatternRewriter &rewriter,
    ...
    if (rounding == RoundingMode::RTZ) {
      ...
      auto as_int32 = bitcast(v, i32_ty);
      auto shifted = lshr(i32_ty, as_int32, i32_val(16));
      auto and_one = and_(i32_ty, shifted, i32_val(1));
      auto add_32767 = add(i32_ty, as_int32, i32_val(32767));
      auto add_value = add(i32_ty, add_32767, and_one);
      auto shifted_add = lshr(i32_ty, add_value, i32_val(16));
      auto truncated = trunc(i16_ty, shifted_add);
      return (bitcast(truncated, i16_ty));
    } else {
      ...
      auto ret = mlir::LLVM::createBuiltinFunc<T>(rewriter, loc, op, funcName,
                                                  i16_ty, valueRange);
      return ret;
    }
  }
  ...
  Fp32_to_Fp8E5M2_RTNE_SW(Location loc, ConversionPatternRewriter &rewriter,
    ...
    for (size_t i = 0; i < 2; ++i) {
      Value fp32 = v[i];
      Value i32 = bitcast(fp32, i32_ty);

      Value s = and_(i32_ty, i32, i32_val(0x80000000));
      Value exp = and_(i32_ty, lshr(i32_ty, i32, i32_val(23)), i32_val(0xFF));
      Value man = and_(i32_ty, i32, i32_val(0x007FFFFF));

      // Convert 8-bit exponent to 5-bit
      Value exp5 = select(icmp_ult(exp, i32_val(0x71)), i32_val(0),
                          sub(i32_ty, exp, i32_val(0x70)));
      ...
      //                           0x00600000 (3/4),
      //                           0x00800000 (1)
      man = select(icmp_ult(exp, i32_val(0x6e)), i32_val(0), man);
      man = select(
          icmp_eq(exp, i32_val(0x6e)),
          select(icmp_ne(man, i32_val(0)), i32_val(0x00200000), i32_val(0)),
          man);
      man = select(icmp_eq(exp, i32_val(0x6f)),
                   select(icmp_uge(man, i32_val(0x00400000)),
                          i32_val(0x00400000), i32_val(0x00200000)),
                   man);
      man = select(icmp_eq(exp, i32_val(0x70)),
                   select(icmp_ugt(man, i32_val(0x00200000)),
                          select(icmp_uge(man, i32_val(0x00600000)),
                                 i32_val(0x00800000), i32_val(0x00600000)),
                          i32_val(0x00400000)),
                   man);

      // Round 23-bit mantissa to 2-bit nearest, ties to even
      Value sig = or_(i32_ty, shl(i32_ty, exp5, i32_val(23)), man);
      Value bias =
          add(i32_ty,
              lshr(i32_ty, and_(i32_ty, sig, i32_val(0x00200000)), i32_val(21)),
              i32_val(0x000FFFFF));
      i32 = add(i32_ty, sig, bias);
      ...
      i32 = select(
          or_(icmp_ugt(exp, i32_val(0x8E)), icmp_uge(sig, i32_val(0x0F700000))),
          i32_val(0x0F7FFFFF), i32);

      // Handle NaN value by keeping it Nan
      i32 =
          select(and_(icmp_eq(exp, i32_val(0xFF)), icmp_ne(man, i32_val(0x0))),
                 i32_val(0x0FC00000), i32);

      // Add sign bit
      i32 = or_(i32_ty, lshr(i32_ty, s, i32_val(3)), i32);

      // Truncate to 8-bit
      result[i] = trunc(i8_ty, lshr(i32_ty, i32, i32_val(21)));
    }
    return result;
  }
  ...
  static Value downcastToFp8_RTNE_oneValue(Location loc,
    ...
    // Get sign and absolute value
    Value intVal = bitcast(v, srcIntType);
    uint32_t signMask = 1 << (srcWidth - 1);
    Value sign =
        trunc(i8_ty, lshr(and_(intVal, srcFpInfo.toLLVMIntValue(signMask)),
                          srcFpInfo.toLLVMIntValue(srcWidth - 8)));

    uint32_t absoluteMask = signMask - 1;
    intVal = and_(intVal, srcFpInfo.toLLVMIntValue(absoluteMask));

    uint32_t expFullMask = ((1U << srcExponentBits) - 1U) << srcMantissaBits;
    uint32_t mantFullMask = 1U << srcMantissaBits - 1U;
    Value isNaNOrInf =
        icmp_eq(and_(intVal, srcFpInfo.toLLVMIntValue(expFullMask)),
                srcFpInfo.toLLVMIntValue(expFullMask));
    Value notInf = icmp_ne(and_(intVal, srcFpInfo.toLLVMIntValue(mantFullMask)),
                           srcFpInfo.toLLVMIntValue(0));
    Value isNaN = and_(isNaNOrInf, notInf);
    ...
    Value remainingMantissaLSB =
        lshr(and_(intVal, mantissaLSBValue), reducedMantissaValue);
    Value roundingBias =
        add(remainingMantissaLSB, srcFpInfo.toLLVMIntValue(baseRoundingBias));
    Value vFp8 = add(intVal, roundingBias);
    ...
    vFp8 = umax(vFp8, srcFpInfo.toLLVMIntValue(dstMinimal));

    // Adjust exponent bias
    uint32_t expBias = (srcBias - dstBias) << srcMantissaBits;
    vFp8 = sub(vFp8, srcFpInfo.toLLVMIntValue(expBias));

    // Shift right and truncate
    vFp8 = trunc(i8_ty, lshr(vFp8, reducedMantissaValue));
    ...
    Value isOverflow =
        icmp_ugt(intVal, srcFpInfo.toLLVMIntValue(dstMaxOfSrcType));
    vFp8 = select(isOverflow, dstFpInfo.toLLVMIntValue(dstMaxPositive), vFp8);
    ...
    for (int i = lutSize - 1; i >= 0; i--) {
      Value cmp;
      if (i % 2 == 0) {
        cmp = icmp_ule(intVal, srcFpInfo.toLLVMIntValue(halfwayPointsLUT[i]));
      } else {
        cmp = icmp_ult(intVal, srcFpInfo.toLLVMIntValue(halfwayPointsLUT[i]));
      }
    ...
    vFp8 = select(isNaN, dstFpInfo.toLLVMIntValue(positiveNan), vFp8);
    ...
    if constexpr (isFp8UZ) {
      Value isNegativeZero =
          and_(icmp_eq(vFp8, i8_val(0x80)), icmp_eq(isNaN, i1_val(0)));
      vFp8 = select(isNegativeZero, i8_val(0), vFp8);
    }
  ...
  convertFp32DowncastCommon(Location loc, ConversionPatternRewriter &rewriter,
                            ...
                            const int &exp_bias) {
    Value as_int32 = bitcast(v, i32_ty);
    int exp_bias_fp32 = 127;
    int exp_shift = exp_bias_fp32 - exp_bias;
    // extract sign, exp and mantissa
    Value sign = and_(as_int32, i32_val(0x80000000));
    Value mantissa = and_(as_int32, i32_val(0x007fffff));
    Value exp = and_(lshr(as_int32, i32_val(23)), i32_val(0xff));

    mantissa = select(icmp_eq(exp, int_val(32, 0)), mantissa,
                      add(mantissa, int_val(32, 0x800000)));
    exp = select(icmp_eq(exp, int_val(32, 0)), exp, sub(exp, int_val(32, 1)));

    if (rounding == RoundingMode::RTNE) {
      int shift_bits = 23 - mantissa_bits;
      int rounding_bias = (1 << (shift_bits - 1)) - 1;
      mantissa = add(i32_ty, mantissa, i32_val(rounding_bias));
    }

    // process denormal cases
    Value new_mantissa =
        select(icmp_ule(exp, int_val(32, exp_shift - 16)), int_val(32, 0),
               lshr(mantissa, int_val(32, 23 - mantissa_bits)));
    Value new_exp =
        select(icmp_ule(exp, int_val(32, exp_shift - 16)),
               int_val(32, exp_shift), exp); // too small for fp16 to express
    new_mantissa = select(icmp_ugt(new_exp, int_val(32, exp_shift - 8)),
                          new_mantissa, lshr(new_mantissa, int_val(32, 8)));
    new_exp = select(icmp_ugt(new_exp, int_val(32, exp_shift - 8)), new_exp,
                     add(new_exp, int_val(32, 8)));
    new_mantissa = select(icmp_ugt(new_exp, int_val(32, exp_shift - 4)),
                          new_mantissa, lshr(new_mantissa, int_val(32, 4)));
    new_exp = select(icmp_ugt(new_exp, int_val(32, exp_shift - 4)), new_exp,
                     add(new_exp, int_val(32, 4)));
    new_mantissa = select(icmp_ugt(new_exp, int_val(32, exp_shift - 2)),
                          new_mantissa, lshr(new_mantissa, int_val(32, 2)));
    new_exp = select(icmp_ugt(new_exp, int_val(32, exp_shift - 2)), new_exp,
                     add(new_exp, int_val(32, 2)));
    new_mantissa = select(icmp_ugt(new_exp, int_val(32, exp_shift - 1)),
                          new_mantissa, lshr(new_mantissa, int_val(32, 1)));
    new_exp = select(icmp_ugt(new_exp, int_val(32, exp_shift - 1)), new_exp,
                     add(new_exp, int_val(32, 1)));
    new_exp = sub(new_exp, i32_val(exp_shift));

    // concat sign, exp and mantissa
    int target_dtype_bits = 1 + exp_bits + mantissa_bits;
    Value new_sign = lshr(sign, i32_val(32 - target_dtype_bits));
    new_exp = shl(new_exp, i32_val(mantissa_bits));
    Value new_value = add(i32_ty, add(i32_ty, new_sign, new_exp), new_mantissa);
    if (target_dtype_bits == 16) {
      return trunc(i16_ty, new_value);
    }
    return trunc(i8_ty, new_value);
  }
  ...
  convertFp32ToFp16(Location loc, ConversionPatternRewriter &rewriter,
    ...
    if (rounding == RoundingMode::RTZ) {
      ...
      Value new_mantissa =
          select(icmp_ule(exp, int_val(32, exp_shift - 16)), int_val(32, 0),
                 lshr(mantissa, int_val(32, 23 - 10)));
      Value new_exp =
          select(icmp_ule(exp, int_val(32, exp_shift - 16)),
                 int_val(32, exp_shift), exp); // too small for fp16 to express
      new_mantissa = select(icmp_ugt(new_exp, int_val(32, exp_shift - 8)),
      ...
      Value new_sign = lshr(sign, i32_val(16));
      new_exp = shl(new_exp, i32_val(10));
      Value new_value =
          add(i32_ty, add(i32_ty, new_sign, new_exp), new_mantissa);
      Value truncated = trunc(i16_ty, new_value);
      return bitcast(truncated, f16_ty);
    }
  ...
  static SmallVector<Value> PK_Fp_to_Fp_Builtin(
    ...
    for (unsigned i = 0; i < num_vec; i++) {
      Value src = undef(srcTy);
      for (unsigned p = 0; p < pk_vec; p++) {
        src = insert_element(src, v[i * pk_vec + p], i32_val(p));
      }
      if (srcTy != srcInstTy)
        src = bitcast(src, srcInstTy);
      Value ret;
      if (isCfg) {
        ValueRange valueRange(
            {src, rounding_mode, denormal_mode, src_modifiers, clamp, ovfl});
        ret = mlir::LLVM::createBuiltinFunc<triton::FpToFpOp>(
            rewriter, loc, op, instr, retInstTy, valueRange);
      } else {
      ...
      if (retTy != retInstTy)
        ret = bitcast(ret, retTy);
      for (unsigned j = 0; j < pk_vec; j++) {
        result.push_back(extract_element(retElemTy, ret, i32_val(j)));
      }
    }
    if (rem_vec) {
      Value src = undef(srcTy);
      ...
      if (srcElemTy.isF32()) {
        ...
        _zero = int_val(bits, 0);
      } else {
        assert(false && "unsupport srcElemTy");
      }
      for (unsigned m = 0; m < pk_vec; m++) {
        if (m < rem_vec) {
          src = insert_element(src, v[num_vec * pk_vec + m], i32_val(m));
        } else {
          src = insert_element(src, _zero, i32_val(m));
        }
      }
      if (srcTy != srcInstTy)
        src = bitcast(src, srcInstTy);
      ...
      for (unsigned n = 0; n < rem_vec; n++) {
        result.push_back(extract_element(retElemTy, ret, i32_val(n)));
      }
    }
    return result;
  }
  ...
  Fp32_to_Fp8E4M3FN_RTNE_HW(Location loc, ConversionPatternRewriter &rewriter,
    ...
    Value rounding_mode = i32_val(0); // ``_MACA_FROUND_TO_NEAREST_INT``
    Value denormal_mode = i32_val(3); // ``_MACA_DENORMAL_FLUSH_NONE``
    Value src_modifiers = i32_val(0); // ``_MACA_SRC_NO_MODIFIER``
    Value clamp = i1_val(0);
    Value ovfl = i1_val(0);
    int pk_vec = 4;
  ...
  Fp8E4M3FN_to_Fp16_HW(Location loc, ConversionPatternRewriter &rewriter,
                       const SmallVector<Value> &v, triton::FpToFpOp op) {
    // TODO(MACA): llvm.mxc.cvt.pk.f8tobf16.fast will increase perf but no NAN
    // handle.
  ...
  Bf16_to_Fp8E4M3FN_RTNE_SW(Location loc, ConversionPatternRewriter &rewriter,
    ...
    SmallVector<Value> buf;
    for (Value val : v) {
      buf.push_back(convertBf16ToFp32(loc, rewriter, val));
    }
    return Fp32_to_Fp8E4M3FN_RTNE_SW(loc, rewriter, buf, op);
  }
  ...
  Fp8E4M3FN_to_Bf16_SW(Location loc, ConversionPatternRewriter &rewriter,
    ...
    for (Value &val : buf) {
      ret.push_back(
          convertFp32ToBf16<triton::FpToFpOp>(loc, op, rewriter, val));
    }
    return ret;
  }
  ...
  SmallVector<Value> createDestOps(triton::FpToFpOp op, OpAdaptor adaptor,
    ...
    if (srcElementType.isF32() && dstElementType.isBF16()) {
      ...
      outVals.reserve(operands[0].size());
      for (Value v : operands[0]) {
        outVals.push_back(convertFp32ToBf16<triton::FpToFpOp>(
            loc, op, rewriter, v, roundingMode.value()));
      }
      return outVals;
    }
    ...
    bool useFP16IntermediateSrc =
        ...
        !(dstElementType.isFloat8E4M3FNUZ() ||
          ...
          dstElementType.isFloat8E5M2FNUZ() || dstElementType.isFloat8E5M2());
    bool useFP32IntermediateSrcF16 =
        srcElementType.isF16() &&
        (dstElementType.isFloat8E4M3FNUZ() || dstElementType.isFloat8E4M3FN() ||
         dstElementType.isFloat8E5M2FNUZ() || dstElementType.isFloat8E5M2()) &&
        !(enableFastF8Cvt);
    bool useFP32IntermediateSrcBF16 =
        srcElementType.isBF16() &&
        (dstElementType.isFloat8E4M3FNUZ() || dstElementType.isFloat8E4M3FN() ||
         dstElementType.isFloat8E5M2FNUZ() || dstElementType.isFloat8E5M2()) &&
        !(enableFastF8Cvt);
...
Value EmitDualBF16ElementwiseOp(Location loc,
  ...
  return FpToFpOpConversion::convertFp32ToBf16<OP>(
      loc, dyn_cast<OP>(result.getOperation()), rewriter, result);
}
...
struct FDivOpConversion
  ...
  SmallVector<Value> createDestOps(mlir::arith::DivFOp op, OpAdaptor adaptor,
    ...
    if (std::getenv("TRITON_ENABLE_RCP_DIVF") !=
        nullptr) { // TRITON_ENABLE_RCP_DIVF with higher priority
      instructionName = "llvm.mxc.rcp.f32";
      StringRef funcName(instructionName);
      if (dtype.isF32()) {
        ValueRange valueRange({operands[0][1]});
        auto ret = mlir::LLVM::createBuiltinFunc<mlir::arith::DivFOp>(
            rewriter, loc, op, funcName, dtype, valueRange);
        return {
        ...
        auto ret_ = mlir::LLVM::createBuiltinFunc<mlir::arith::DivFOp>(
            rewriter, loc, op, funcName, dtype, valueRange);
        auto ret = FpToFpOpConversion::convertFp32ToFp16(loc, rewriter, ret_);
        return {
            rewriter.create<LLVM::FMulOp>(loc, elemTy, operands[0][0], ret)};
      } else {
        auto v0 = FpToFpOpConversion::convertBf16ToFp32(loc, rewriter,
                                                        operands[0][0]);
        auto v1 = FpToFpOpConversion::convertBf16ToFp32(loc, rewriter,
                                                        operands[0][1]);
        ValueRange valueRange({v0, v1});
        auto ret_ = mlir::LLVM::createBuiltinFunc<mlir::arith::DivFOp>(
            rewriter, loc, op, funcName, dtype, valueRange);
        auto ret = FpToFpOpConversion::convertFp32ToBf16<mlir::arith::DivFOp>(
            loc, op, rewriter, ret_);
        return {
            rewriter.create<LLVM::FMulOp>(loc, elemTy, operands[0][0], ret)};
      }
      ...
      if (dtype.isF32()) {
        ...
        auto ret = mlir::LLVM::createBuiltinFunc<mlir::arith::DivFOp>(
            rewriter, loc, op, funcName, dtype, valueRange);
        return {FpToFpOpConversion::convertFp32ToBf16<mlir::arith::DivFOp>(
            loc, op, rewriter, ret)};
      }
    } else {
  ...
  SmallVector<Value> createDestOps(mlir::arith::MulFOp op, OpAdaptor adaptor,
    ...
    if (lhsElemTy.isBF16() && rhsElemTy.isBF16()) {
      ...
      if (std::getenv("TRITON_DISABLE_ELEMENTWISE_PK_FMA_OPT") == nullptr &&
        ...
        Value vec0 = undef(vecType);
        Value vec1 = undef(vecType);
        Value vec2 = undef(vecType);
        Value zero = f32_val(0.0);
        vec0 = insert_element(vec0, operands[0][0], i32_val(0));
        vec0 = insert_element(vec0, operands[1][0], i32_val(1));
        vec1 = insert_element(vec1, operands[0][1], i32_val(0));
        vec1 = insert_element(vec1, operands[1][1], i32_val(1));
        vec2 = insert_element(vec2, zero, i32_val(0));
        vec2 = insert_element(vec2, zero, i32_val(1));
        StringRef funcName("llvm.mxc.pk.fma.f32");
        ValueRange valueRange({vec0, vec1, vec2});
        auto ret = mlir::LLVM::createBuiltinFunc<arith::MulFOp>(
            rewriter, loc, op, funcName, vecType, valueRange);
        return {extract_element(ret, i32_val(0)),
                extract_element(ret, i32_val(1))};
      }
  ...
  SmallVector<Value> createDestOps(arith::AddFOp op, OpAdaptor adaptor,
    ...
    if (lhsElemTy.isBF16() && rhsElemTy.isBF16()) {
      ...
      if (std::getenv("TRITON_DISABLE_ELEMENTWISE_PK_FMA_OPT") == nullptr &&
        ...
        Value vec1 = undef(vecType);
        Value vec2 = undef(vecType);
        Value one = f32_val(1.0);
        vec0 = insert_element(vec0, operands[0][0], i32_val(0));
        vec0 = insert_element(vec0, operands[1][0], i32_val(1));
        vec1 = insert_element(vec1, one, i32_val(0));
        vec1 = insert_element(vec1, one, i32_val(1));
        vec2 = insert_element(vec2, operands[0][1], i32_val(0));
        vec2 = insert_element(vec2, operands[1][1], i32_val(1));
        StringRef funcName("llvm.mxc.pk.fma.f32");
        ValueRange valueRange({vec0, vec1, vec2});
        auto ret = mlir::LLVM::createBuiltinFunc<arith::AddFOp>(
            rewriter, loc, op, funcName, vecType, valueRange);
        return {extract_element(ret, i32_val(0)),
                extract_element(ret, i32_val(1))};
      }
...
struct SIToFPOpConversion
  ...
  SmallVector<Value> createDestOps(mlir::arith::SIToFPOp op, OpAdaptor adaptor,
    ...
    // When outElemTy is bf16, inElemTy is int8 and operand size >= 4,
    // NV will use its proprietary 'prmt.b32' instruction for type conversion.
    // But there is no proprietary 'prmt.b32' instruction on c500,
    // therefore, for this case, we first convert int8 to f32 then to bfloat16.
    if (outElemTy.isBF16()) {
      auto value = rewriter.create<LLVM::SIToFPOp>(loc, f32_ty, operands[0][0]);
      return {FpToFpOpConversion::convertFp32ToBf16<LLVM::SIToFPOp>(
          loc, dyn_cast<LLVM::SIToFPOp>(value.getOperation()), rewriter,
          value)};
    } else {
...
struct TruncFOpConversion
  ...
  SmallVector<Value> createDestOps(mlir::arith::TruncFOp op, OpAdaptor adaptor,
    ...
    if (outElemTy.isBF16()) {
      ...
      return {FpToFpOpConversion::convertFp32ToBf16<mlir::arith::TruncFOp>(
          loc, op, rewriter, operands[0][0])};
    } else {
...
//                               int computeCapability,
//                               PatternBenefit benefit = patternBenefitDefault)
//       : ElementwiseOpConversionBase(typeConverter, axisAnalysisPass,
//       benefit),
//         computeCapability(computeCapability) {}
//
...
//                                    ConversionPatternRewriter &rewriter,
//                                    Type elemTy, MultipleOperandsRange
//                                    operands, Location loc) const {
//     if (isClipPattern(op)) {
