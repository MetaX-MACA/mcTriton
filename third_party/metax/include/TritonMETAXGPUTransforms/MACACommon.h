...
T getSubLinearIndex(llvm::ArrayRef<T> multiDimIndex, llvm::ArrayRef<T> shape,
  ...
  return getSubLinearIndexImpl<T>(
      mlir::triton::applyPermutation(multiDimIndex, order),
      mlir::triton::applyPermutation(shape, order));
}
...
void genSwiMask(Value dep, unsigned inVec,
                ...
                ArrayRef<int64_t> subSizes, ArrayRef<int64_t> tileIdx,
...
/// collect all valid loads and deps of dots op.
