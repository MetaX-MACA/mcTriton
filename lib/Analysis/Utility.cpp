...
SmallVector<unsigned> getParentThreadsPerWarp(Attribute layout) {
  if (auto sliceEncoding = mlir::dyn_cast<SliceEncodingAttr>(layout)) {
    return getParentThreadsPerWarp(sliceEncoding.getParent());
  }
...
bool ReduceOpHelper::isReductionOnLayoutFastAxis() {
  auto order = getParentOrder(getSrcLayout());
  auto srcLayout = getSrcLayout();
#ifdef USE_MACA
  if (auto mmaLayout = mlir::dyn_cast<triton::gpu::MACAMmaEncodingAttr>(srcLayout)) {
    if (mmaLayout.getColMajor() != 0) {
      order = {0, 1};
    }
  } 
#endif
  return getParentAxis(srcLayout, axis) == order[0];
}
...
unsigned ReduceOpHelper::getThreadOffsetOnReductionAxis() {
  ...
  // Common algorithm:
  // compute the cumprod of threadPerWarp whose dim order smaller than reduce dim.
  // instance 1: there is a src tensor: BlockLayout {shape=[2, 4, 8], threadsPerWarp=[2, 4, 8], order=[2, 1, 0]}
  // we wanna reduce along dim0, threadOffset = threadPerWarp[order[0]] * threadPerWarp[order[1]] = 8 * 4  = 32
  // 
  // instance 2: For the above src Tensor, we wanna first reduce along dim1, then reduce dim0
  // step1: reduce along dim1, threadOffset = threadPerWarp[order[0]] = 8, we get the following dst layout:
  // SliceLayout{shape=[2, 4], threadsPerWarp=[2, 32], order=[1, 0]}
  // step2: reduce along dim0, threadOffset = threadPerWarp[order[0]] = 32, we get the following dst layout:
  // SliceLayout{shape=[2], threadsPerWarp=[64], order=[0]}
  unsigned threadOffset = 1;
  auto order = getOrder(srcLayout);
  SmallVector<unsigned> threadsPerWarp = getThreadsPerWarp(srcLayout);
#ifdef USE_MACA
  if (auto mmaLayout = mlir::dyn_cast<triton::gpu::MACAMmaEncodingAttr>(srcLayout)) {
    if (mmaLayout.getColMajor() != 0) {
      order = {0, 1};
  ...
  // The above algorithm can satisfy all cases unless we reduce along fastest moving dim twice,
  // we should multiply threadOffset with parentThreadsPerWarp[parentReduceDim] to solve this issue.
  //
  // for instance, there is a src tensor: BlockLayout {shape=[2, 2, 2], threadsPerWarp=[2, 2, 2], order=[2, 1, 0]}
  // src data value: [[a, b], [c, d]], [[e, f], [g, h]]] (each thread own one element)
  // we wanna first reduce along dim2 then reduce along dim1 to get a Tensor 
  // with value: [[abcd, abcd, abcd, abcd], [efgh, efgh, efgh, efgh]].
  //
  // after reducing along dim 2, we get sliceLayout: 
  // {shape=[2, 2], threadsPerWarp=[2, 4], order=[1, 0]}
  // value become: [[ab, ab, cd, cd], [ef, ef, gh, gh]] (each thread own one element)
  //
  // then we reduce along dim1, threadOffset = 1 according to the common alogorithm
  // dst data value may become: [[abab, abab, cdcd, cdcd], [efef, efef, ghgh, ghgh]]
  // which doesn't meet our expectation.
  //
  // According to algorithm2, we multiply above threadOffset with parentThreadsPerWarp[parentReduceDim] = 2
  // then dst value become: [[abcd, abcd, abcd, abcd], [efgh, efgh, efgh, efgh]], which meet our expectation.
  //
  // verification case:
  // maca/maca_tests/pytest_unit_test/language/test_standard.py::test_topk
  // maca/maca_tests/pytest_unit_test/language/test_standard.py::test_sort
  if (auto sliceLayout = mlir::dyn_cast<SliceEncodingAttr>(srcLayout)) {
    int currentReduceDimOrder = getOrder(sliceLayout)[axis];
    auto parentLayout = sliceLayout.getParent();
    int parentReduceDimOrder = getOrder(parentLayout)[sliceLayout.getDim()];
    auto parentThreadsPerWarp = getThreadsPerWarp(parentLayout);
    
    if (parentReduceDimOrder == currentReduceDimOrder && parentReduceDimOrder == 0) {
      threadOffset *= parentThreadsPerWarp[sliceLayout.getDim()];
    }
  } 
  return threadOffset;
}
...
bool supportMMA(triton::DotOp op, int major, int minor) {
  ...
  if (aElemTy.isF32() && bElemTy.isF32()) {
    return op.getInputPrecision() == InputPrecision::TF32 || (major == 2 && minor == 0);
  }
  return supportMMA(op.getA(), major, minor) && supportMMA(op.getB(), major, minor);
#else
...
bool supportMMA(Value value, int major, int minor) {
    ...
    return elemTy.isF16() || elemTy.isBF16() || 
           (elemTy.isF32() && major >= 2) || (elemTy.isF64() && major >= 2) ||
           (elemTy.isInteger(8) && major >= 2) || (elemTy.isFloat8E5M2() && major >=2 && minor >= 6) ||
           (elemTy.isFloat8E4M3FN() && major >=2 && minor >= 6);
#else
...
static bool isMACAMmaToMmaShortcut(Attribute srcEncoding, Attribute dstEncoding) {
  auto src = dyn_cast<MACAMmaEncodingAttr>(srcEncoding);
  auto dst = dyn_cast<MACAMmaEncodingAttr>(dstEncoding);
  if (!src || !dst)
    return false;
  // when #mma = MmaEncoding<version=3, warpsPerCTA=[..., 1]>
  return src && dst && src.getVersionMajor() == 3 &&
...
bool isMACAMmaToMmaShortcut(RankedTensorType srcTy, RankedTensorType dstTy) {
  return isMACAMmaToMmaShortcut(srcTy.getEncoding(), dstTy.getEncoding());
}
...
bool isMmaToDotShortcut(RankedTensorType srcTy, RankedTensorType dstTy) {
  ...
  auto mmaLayout = mlir::cast<triton::gpu::MACAMmaEncodingAttr>(srcLayout);
  auto dotOperandLayout = mlir::cast<triton::gpu::DotOperandEncodingAttr>(dstLayout);
  auto dotOperandLayoutMma = mlir::cast<triton::gpu::MACAMmaEncodingAttr>(dotOperandLayout.getParent());
  // if the mma is followed by reudce, it may use shared memory
  // TODO: if add more dtype should modify the condition
  bool isWarpMatch = ((mmaLayout.getWarpsPerCTA()[1] == 1) || 
                      (mmaLayout.getWarpsPerCTA()[1] > 1 && dstShape[1] == 16));
  if (!dotOperandLayoutMma)
    return false;
  // we should ensure 4 * n (mma) == k (dotoperand mma)
  // TODO: support shortcut b and more data type
  return mmaLayout.getVersionMajor() == 2 &&
         isWarpMatch &&
         dotOperandLayout.getOpIdx() == 0 &&
         // maybe no need this, but maybe cause convert mma to mma
         dotOperandLayoutMma == mmaLayout &&
         (srcTy.getElementType().isF16() || srcTy.getElementType().isBF16()) &&
         mmaLayout.getColMajor() == 1 && 
         mmaLayout.getElementsMNK()[1] * 4 == dotOperandLayoutMma.getElementsMNK()[2] && 
