...
#ifdef USE_MACA
  if (auto srcMmaLayout = mlir::dyn_cast<MACAMmaEncodingAttr>(srcLayout)) {
    if (mlir::isa<DotOperandEncodingAttr>(dstLayout)) {
      if (isMmaToDotShortcut(srcTy, dstTy)) {
        return {};
      }
    } else if (auto dstMmaLayout =
                   mlir::dyn_cast<MACAMmaEncodingAttr>(dstLayout)) {
      if (isMACAMmaToMmaShortcut(srcTy, dstTy)) {
        return {};
      }
    }
  }
...
#ifdef USE_MACA
  if (auto mma = mlir::dyn_cast<MACAMmaEncodingAttr>(srcLayout)) {
    if (mma.getColMajor() == 0 && mma.getIsBTrans() &&
...
#ifdef USE_MACA
  if (!op.getWithPad() || std::getenv("TRITON_DISABLE_MACA_C_STRORE_PAD") != nullptr) {
    repShape[paddedDim] += 0;
  } else {
...
private:
  ...
  void run() {
    ...
    if(std::getenv("TRITON_ENABLE_PRINT_ALLOC_INFO") != nullptr){
      for (auto iter : bufferRange) {
        std::cout << (int)iter.first->kind << ", " << iter.first->id << ", " << iter.first->size << ", " << iter.first->offset << ": " << iter.second.start() << ", " << iter.second.end() << ", " << iter.second.size() << std::endl;
      }
    }
  }
  ...
  void getScratchValueSize(Operation *op) {
    ...
    if (auto reduceOp = dyn_cast<triton::ReduceOp>(op)) {
      ...
      auto bytes = product(srcType.getShape()) *
