...
void MembarAnalysis::update(Operation *op, BlockInfo *blockInfo,
  ...
  if (!disableOptMACA) {
    //TODO(MACA)
    // if (std::getenv("TRITON_ENABLE_MACA_MEMBAR_OPT0")) {
    //   // TODO(MACA): PipelineMACA5 cvt(S0->R) + insert_slice(R->S1) to
    //   // be supported, enable InsertSliceOp membar analysis after
    //   // supported.
    //   if (isa<tensor::InsertSliceOp>(op)) return;
    // }
    if (isa<triton::gpu::LocalLoadOp>(op)) {
      auto cvtOp = dyn_cast<triton::gpu::LocalLoadOp>(op);
      bool isIntrinsic = cvtOp.getIntrinsic();
      if (isIntrinsic) return;
    }
    if (isa<triton::gpu::LocalStoreOp>(op)) {
      auto cvtOp = dyn_cast<triton::gpu::LocalStoreOp>(op);
      bool isIntrinsic = cvtOp.getIntrinsic();
      if (isIntrinsic) return;
    }
    ...
    if (isa<triton::gpu::AsyncCopyGlobalToLocalOp>(op)) {
      auto insertAsyncOp = dyn_cast<triton::gpu::AsyncCopyGlobalToLocalOp>(op);
      bool isIntrinsic = insertAsyncOp.getIntrinsic();
      if (isIntrinsic) return;
    }
  }
#endif
...
#ifdef USE_MACA
  if (isa<gpu::BarrierOp>(op) || isa<triton::gpu::BarrierSharedOp>(op) || isa<triton::gpu::BarrierOp>(op)) {
#else
...
#ifdef USE_MACA
      !(isa<gpu::BarrierOp>(op->getNextNode()) || isa<triton::gpu::BarrierSharedOp>(op->getNextNode()) || isa<triton::gpu::BarrierOp>(op->getNextNode()))) {
#else
