...
private:
  ...
  promoteOperands(triton::CallOp callOp,
    ...
    if (!caller->hasAttr("allocation.offset")) {
      ...
      auto base = LLVM::getStackPointer(rewriter, caller, getTypeConverter());
      base = bitcast(base, ptr_ty(rewriter.getContext(), i8_ty, 3));
#else
...
#ifndef USE_MACA_OPAQUE_PTR
    auto sharedMemoryBase = LLVM::getSharedMemoryBase(callOp->getLoc(), rewriter, callOp, getTypeConverter());
    sharedMemoryBase = bitcast(sharedMemoryBase, ptr_ty(rewriter.getContext(), i8_ty, 3));
    promotedOperands.push_back(sharedMemoryBase);
#else
