...
public:
  ...
  SmallVector<Value> getSmemBases(SourceOp op, unsigned elems,
    ...
    indexToBase[indices[0]] =
        ...
        bitcast(LLVM::getSharedMemoryBase(loc, rewriter, op.getOperation(), getTypeConverter()), 
                ptr_ty(rewriter.getContext(), getElementType(op, indices[0]), 3));
    #else
        LLVM::getSharedMemoryBase(loc, rewriter, op.getOperation());
    #endif
    for (unsigned i = 1; i < op.getNumOperands(); ++i) {
    #ifndef USE_MACA_OPAQUE_PTR
      auto elemPtrTy = ptr_ty(rewriter.getContext(), getElementType(op, indices[i]), 3);
      auto lastElemPtrTy = ptr_ty(rewriter.getContext(), getElementType(op, indices[i-1]), 3);
      indexToBase[indices[i]] = bitcast(gep(lastElemPtrTy, getElementType(op, indices[i - 1]), indexToBase[indices[i - 1]], i32_val(elems)),
                  elemPtrTy);
    #else
