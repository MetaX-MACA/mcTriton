...
static SmallVector<Value> computeWarpLevelHistogram(
  ...
  for (int i = 0; i < numElementsPerThreads; ++i) {
    ...
    for (int j = 0; j < numBits; ++j) {
      ...
      Value bit =
#ifdef USE_MACA
          targetInfo.ballot(rewriter, loc, int_ty(numThreadPerWarp), cmp, op);
#else
...
static SmallVector<Value> computeCrossWarpHistogram(
  ...
  Type i32_ptr_ty = ptr_ty(rewriter.getContext(), i32_ty, 3);
  Value baseSharedMemPtrI32 = bitcast(baseSharedMemPtr, i32_ptr_ty);
#endif
...
#ifndef USE_MACA_OPAQUE_PTR
    Value sharedMemPtr =
        bitcast(gep(baseSharedMemPtrI32.getType(), i32_ty, baseSharedMemPtrI32, offset), i32_ptr_ty);
#else
    Value sharedMemPtr =
  ...
  for (Value index : indices) {
    Value sharedMemPtr =
#ifndef USE_MACA_OPAQUE_PTR
        bitcast(gep(baseSharedMemPtrI32.getType(), i32_ty, baseSharedMemPtrI32, index), i32_ptr_ty);
#else
...
public:
  ...
  matchAndRewrite(triton::HistogramOp op, OpAdaptor adaptor,
    ...
    Value baseSharedMemPtr =
          LLVM::getSharedMemoryBase(loc, rewriter, op.getOperation(), getTypeConverter());
  #else
    Value baseSharedMemPtr =
