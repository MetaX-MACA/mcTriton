...
Type TritonGPUToLLVMTypeConverter::convertTritonPointerType(
  ...
  if (isa<RankedTensorType>(pointeeType)) {
    ...
    types.push_back(LLVM::LLVMPointerType::get(ctx, convertType(eleType), type.getAddressSpace()));
  #else
...
#ifndef USE_MACA_OPAQUE_PTR
  return LLVM::LLVMPointerType::get(ctx, convertType(pointeeType), type.getAddressSpace());
#else
...
#ifdef USE_MACA
   if (mlir::dyn_cast<MACAMmaEncodingAttr>(dotOpLayout.getParent())) {
    ...
    if (targetTyMap.count(bitwidth)) {
      return targetTyMap.lookup(bitwidth);
    } else {
...
Type TritonGPUToLLVMTypeConverter::convertTritonTensorType(
  ...
  if (auto shared_layout = mlir::dyn_cast<SharedEncodingAttr>(layout)) {
    ...
    // base ptr
  #ifndef USE_MACA_OPAQUE_PTR
    auto ptrType = LLVM::LLVMPointerType::get(ctx, eltType, 3);
  #else
...
Type TritonGPUToLLVMTypeConverter::convertMemDescType(MemDescType type) {
  ...
  // base ptr
#ifndef USE_MACA_OPAQUE_PTR
  auto ptrType = LLVM::LLVMPointerType::get(ctx, convertType(type.getElementType()), 3);
#else 
  auto ptrType = LLVM::LLVMPointerType::get(ctx, 3);
#endif
  types.push_back(ptrType);
