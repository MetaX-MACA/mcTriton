...
private:
  bool isSupported(Attribute srcLayout, Attribute dstLayout) const {
    ...
    return isaDistributedLayout(srcLayout) && isaDistributedLayout(dstLayout) &&
           !isLayoutMmaMACA(srcLayout) && !isLayoutMmaMACA(dstLayout);
#else
    return isaDistributedLayout(srcLayout) && isaDistributedLayout(dstLayout) &&
  ...
  void processReplica(Location loc, ConversionPatternRewriter &rewriter,
    ...
    for (unsigned ctaId = 0; ctaId < accumNumCTAsEachRep; ++ctaId) {
      ...
      #ifndef USE_MACA_OPAQUE_PTR
        ptr = bitcast(ptr, ptr_ty(rewriter.getContext(), vecTy, 3));
      #else
  ...
  lowerDistributedToDistributed(triton::gpu::ConvertLayoutOp op,
    ...
    // ptr type is not supported, here get i64_ty if it's ptr
    elemTy = isa<triton::PointerType>(elemTy) ? i64_ty : elemTy;
    elemTy = getTypeConverter()->convertType(elemTy);;
    auto elemPtrTy = ptr_ty(rewriter.getContext(), elemTy, 3);
  #else
    Value smemBase =
