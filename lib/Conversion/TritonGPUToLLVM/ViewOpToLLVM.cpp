  ...
  matchAndRewrite(TransOp op, OpAdaptor adaptor,
    ...
    if (auto enc = dyn_cast<SharedEncodingAttr>(resultTy.getEncoding())) {
      ...
      auto dstSmemObj = SharedMemoryObject(
          ...
          /*offsets=*/applyPermutation(srcSmemObj.offsets, op.getOrder()), srcSmemObj.static_offset);
#else
      auto dstSmemObj = SharedMemoryObject(
          srcSmemObj.base, srcSmemObj.baseElemType,
          /*strides=*/applyPermutation(srcSmemObj.strides, op.getOrder()),
...
struct MemDescSubviewOpConversion
  ...
  matchAndRewrite(triton::gpu::MemDescSubviewOp op, OpAdaptor adaptor,
    ...
    if (op.getIsConstantOffs()) {
      ...
      assert(opOffsetVals.size() == 3 && "invalid ExtractSlice size!");
      SmallVector<int64_t> staticOffsets;
      for (auto i = 0; i < opOffsetVals.size(); ++i) {
        if (auto constant = dyn_cast<arith::ConstantOp>(
                                     opOffsetVals[i].getDefiningOp())) {
          int64_t intVal = mlir::cast<IntegerAttr>(constant.getValue())
                                .getValue()
                                .getSExtValue();
          staticOffsets.push_back(intVal);
          
        } else {
          assert(false && "isConstantOffs only support offsets are constant");
        } 
      } 
      strides_.push_back(shape[1] * shape[2]); 
      strides_.push_back(order[0] ? shape[2] : 1);
      strides_.push_back(order[1] ? shape[1] : 1);
      
      for (auto i = 0; i < opOffsetVals.size(); ++i) {
         offset_ += (staticOffsets[i] * strides_[i]);
      }
      offset = i32_val(offset_);
    #ifndef USE_MACA_OPAQUE_PTR
      auto elemPtrTy = ptr_ty(rewriter.getContext(), llvmElemTy, 3);
    #else
      auto elemPtrTy = ptr_ty(rewriter.getContext(), 3);
    #endif
      smemObj = SharedMemoryObject(smemObj.base, llvmElemTy, strides, offsetVals,
                                   offset);
    } else {
      offset = dot(rewriter, loc, opOffsetVals, smemObj.strides);
    #ifndef USE_MACA_OPAQUE_PTR
      auto elemPtrTy = ptr_ty(rewriter.getContext(), llvmElemTy, 3);
    #else
      auto elemPtrTy = ptr_ty(rewriter.getContext(), 3);
    #endif
      smemObj = SharedMemoryObject(gep(elemPtrTy, llvmElemTy, smemObj.base, offset),
