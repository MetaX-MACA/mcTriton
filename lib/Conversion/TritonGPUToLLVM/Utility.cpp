...
#ifdef USE_MACA
          /*offsets=*/{elems.begin() + 1 + rank, elems.end() - 1},
          /*static_offset=*/elems[elems.size()-1]};
#else
...
Value addStringToModule(Location loc, ConversionPatternRewriter &rewriter,
  ...
  Type globalPtrType = LLVM::LLVMPointerType::get(ctx, globalType, global.getAddrSpace());
  Value globalPtr = rewriter.create<LLVM::AddressOfOp>(
      UnknownLoc::get(ctx), globalPtrType, global.getSymName());
  auto ptrTy = LLVM::LLVMPointerType::get(rewriter.getContext(), i8_ty, global.getAddrSpace());
  Value stringStart =
    gep(ptrTy, globalType, globalPtr, SmallVector<Value>({zero, zero}));
#else
