...
#ifdef USE_MACA
                     ScanLoweringHelper &helper, Value laneIdAxis, Operation *op) {
#else
...
#ifdef USE_MACA
        shfl[j] = targetInfo.shuffleUp(rewriter, loc, acc[j], i * threadStride, op);
#else
...
static void storeWarpAccumulator(SmallVector<SmallVector<Value>> &srcValues,
  ...
  for (unsigned srcIndex = 0; srcIndex < srcValues.size(); srcIndex++) {
    ...
    for (unsigned i = 0; i < lastElement.size(); ++i) {
      ...
      Value writePtr = gep(ptr_ty(rewriter.getContext(), smemTypes[i], 3), smemTypes[i],
                           smemBases[i], index);
#else
...
static void AddPartialReduce(SmallVector<SmallVector<Value>> &srcValues,
...
#ifdef USE_MACA
                             Value laneIdAxis, Value parallelLaneId, Operation* op) {
#else
  ...
  for (unsigned srcIndex = 0; srcIndex < srcValues.size(); srcIndex++) {
...
#ifndef USE_MACA_OPAQUE_PTR
        Value ptr =
            gep(ptr_ty(rewriter.getContext(), elemTy, 3), elemTy, smemBases[j], index);
#else
        Value ptr =
    ...
    for (unsigned i = 0; i < helper.getNumOperands(); ++i) {
#ifdef USE_MACA
      auto elem = targetInfo.shuffleUp(rewriter, loc, temp[i], threadStride, op);
#else
...
#ifdef USE_MACA
                                    Value laneIdAxis, Value laneIdLast, Operation* op) {
#else
    ...
    for (unsigned j = 0; j < op.getNumOperands(); ++j) {
      for (unsigned k = iWarpSize / 2; k >= 1; k = k / 2) {
        ...
        srcValues[revIndex][j] =
            targetInfo.shuffleXor(rewriter, loc, srcValues[revIndex][j], k, op);
#else
        srcValues[revIndex][j] =
