...
// reduce MTE instructions by convert multiple divf instructions to a common rcpf instruction
class ExtractCommonRcpFromDivPattern : public mlir::RewritePattern {
public:
    ExtractCommonRcpFromDivPattern(mlir::MLIRContext *context)
    : mlir::RewritePattern(mlir::arith::DivFOp::getOperationName(), 2, context){}

    mlir::LogicalResult match(Operation *op) const override {
      auto divOp = cast<mlir::arith::DivFOp>(op);
      if (!divOp) {
        return mlir::failure();
      }
    ...
    void rewrite(Operation *op, mlir::PatternRewriter &rewriter) const override {
      ...
      if (auto splatOp = llvm::dyn_cast<triton::SplatOp>(rhsOp)) {
        ...
        auto rcpfOp = rewriter.create<triton::RcpfOp>(loc, divend.getType(), divend);
        rcpfResult = rcpfOp.getResult();
      } else if (matchPattern(rhsOp, m_Constant(&constAttr)) &&
                 constAttr.isSplat()) {
        auto value = constAttr.getSplatValue<Attribute>();
        Value divend;
        divend = arith::ConstantOp::materialize(
            rewriter, value, constAttr.getElementType(), loc);
        auto rcpfOp = rewriter.create<triton::RcpfOp>(loc, divend.getType(), divend);
        rcpfResult = rcpfOp.getResult();
      } else {
      ...
      // 2. splat it
      auto newSplat = rewriter.create<triton::SplatOp>(loc, rhs.getType(), rcpfResult);
      auto newSplatResult = newSplat.getResult();
      // 3. create mulf
      auto lhs = divOp.getLhs();
      auto mulOp = rewriter.create<mlir::arith::MulFOp>(loc, rhs.getType(), lhs, newSplatResult);
      // 4. replace origin op
