...
#ifdef USE_MACA
      // convert mma->dotoperand to mma->shared and shared->dotoperand if not satisfy the condition
      // we do not use this convert if satisfy shortcut
      if (auto srcMfmaEncoding =
              dyn_cast<triton::gpu::MACAMmaEncodingAttr>(srcEncoding)) {

        if (isMmaToDotShortcut(srcType, dstType))
          return;
      }
#endif
