...
#ifdef USE_MACA
  } else if (auto macammaLayout = mlir::dyn_cast<MACAMmaEncodingAttr>(layout)) {
    return macammaLayout.getShapePerCTATile(tensorShape);
  } else if (auto macadotLayout = mlir::dyn_cast<DotOperandEncodingAttr>(layout)) {
    return macadotLayout.getShapePerCTATile(tensorShape);
#endif
  } else {
...
int getNumRepK(int K, int elemsPerK) {
  return getNumRepK_(K, elemsPerK);
}
// Get number of elements per thread for $a operand.
size_t getANumElemsPerThread(ArrayRef<int64_t> shape, int wpt,
                             ArrayRef<unsigned> elemsPerThread) {
  int repM = getNumRepM(shape[0], wpt, elemsPerThread[0]);
  int repK = getNumRepK_(shape[1], elemsPerThread[2]);
  // each warp will caculate repM * repK mma, which means
  // every thread of the warp will get repM * repK elements
  // N.B. Only for FP32 MMA because each thread get 1 element
  // every mma.
  return repM * repK * elemsPerThread[0] * elemsPerThread[2];
}

// Get number of elements per thread for $b operand.
size_t getBNumElemsPerThread(ArrayRef<int64_t> shape, int wpt,
                             ArrayRef<unsigned> elemsPerThread) {
  int repK = getNumRepK_(shape[0], elemsPerThread[2]);
  int repN = getNumRepN(shape[1], wpt, elemsPerThread[1]);
  return std::max(repN, 1) * repK * elemsPerThread[2] * elemsPerThread[1];
}
#endif
...
#ifdef USE_MACA
SmallVector<unsigned>
MACAMmaEncodingAttr::getElemsPerThread(ArrayRef<int64_t> shape,
                                       Type eltTy) const {
  size_t rank = shape.size();
  ...
  // TODO(liuyuxin): ElemsPerThread here seems only used to define 
  // elements per thread of C matrix in MACAMMAConversionHelper.loadC.
  // So the size is 4 * elemsCol * elemsRow beacuse each thread gets 
  // 4 elements in C matrix every mma.
  // ceil(shape[i], 16 * getWarpsPerCTA()[i]) means the number of mma in 
  // dim i, and every mma requires [4, 1] regs to save C matrix, 
  // threrefore elemsRow should multiply 4.

  auto tM = getElementsMNK()[0];
  auto tN = getElementsMNK()[1];
  auto colMajor = getColMajor();
  unsigned elemsRow = ceil<unsigned>(shape[0], 16 * getWarpsPerCTA()[0] * tM) * tM;
  unsigned elemsCol = ceil<unsigned>(shape[1], 16 * getWarpsPerCTA()[1] * tN) * tN;
  if (colMajor != 0) {
    elemsCol *= 4;
  } else {
    elemsRow *= 4;
...
SmallVector<int>
MACAMmaEncodingAttr::getThreadShape(bool needTrans) const {
  return getMmaThreadShape(needTrans);
}

SmallVector<unsigned> 
MACAMmaEncodingAttr::getElemsPerThreadOrTrans(triton::gpu::DotOperandEncodingAttr& encoding) const {
...
unsigned MACAMmaEncodingAttr::getTotalElemsPerThread(ArrayRef<int64_t> shape,
                                                     Type eltTy) const {
  return product<unsigned>(getElemsPerThread(shape, eltTy));
}
...
DotOperandEncodingAttr::getElemsPerThread(ArrayRef<int64_t> shape,
  ...
  auto opIdx = getOpIdx();
  auto mmaLayout = 
      mlir::dyn_cast<MACAMmaEncodingAttr>(getParent());
  assert(mmaLayout);
  ...
  int numThreadK = 4;
  // elems for thread MN
  elemsPerThread[opIdx] = ceil<unsigned>(shape[opIdx], numThreadMN * warpPerCTA * tMN) * tMN;
  // elems for thread K
  elemsPerThread[kDim] = ceil<unsigned>(shape[kDim], numThreadK * tK) * tK;
  return elemsPerThread;
}
...
Attribute MACAMmaEncodingAttr::parse(AsmParser &parser, Type type) {
  ...
  for (const NamedAttribute &attr : dict) {
    ...
    if (attr.getName() == "elementsStride") {
      if (parseIntArrayAttr(parser, attr, elementsStride, "elementsStride").failed())
        return {};
    }
  ...
  return parser.getChecked<MACAMmaEncodingAttr>(
      parser.getContext(), versionMajor, versionMinor, warpsPerCTA, elementsMNK, colMajor, *CTALayout, isATrans, isBTrans, elementsStride);
}
...
void MACAMmaEncodingAttr::print(AsmPrinter &printer) const {
  printer << "<{"
          ...
          << ", elementsStride = [" << getElementsStride() << "]";
  maybePrintCTALayout(getContext(), printer, getCTALayout(),
                      /*rank=*/getWarpsPerCTA().size());
  printer << "}>";
}
...
SmallVector<unsigned>
MACAMmaEncodingAttr::getShapePerCTATile(ArrayRef<int64_t> tensorShape) const {
  return {16 * getWarpsPerCTA()[0] * getElementsMNK()[0],
          16 * getWarpsPerCTA()[1] * getElementsMNK()[1]};
}

SmallVector<unsigned> AMDWmmaEncodingAttr::getCTAsPerCGA() const {
  return SmallVector<unsigned>(getCTALayout().getCTAsPerCGA());
}
SmallVector<unsigned> MACAMmaEncodingAttr::getCTAsPerCGA() const {
  return SmallVector<unsigned>(getCTALayout().getCTAsPerCGA());
}
SmallVector<unsigned> AMDWmmaEncodingAttr::getCTAOrder() const {
  return SmallVector<unsigned>(getCTALayout().getCTAOrder());
}
SmallVector<unsigned> MACAMmaEncodingAttr::getCTAOrder() const {
  return SmallVector<unsigned>(getCTALayout().getCTAOrder());
}
SmallVector<unsigned> AMDWmmaEncodingAttr::getCTASplitNum() const {
  return SmallVector<unsigned>(getCTALayout().getCTASplitNum());
}
SmallVector<unsigned> MACAMmaEncodingAttr::getCTASplitNum() const {
  return SmallVector<unsigned>(getCTALayout().getCTASplitNum());
}
SmallVector<unsigned> AMDWmmaEncodingAttr::getWarpsPerCTA() const {
  return SmallVector<unsigned>(getWarpsPerCTA__());
}
SmallVector<unsigned> MACAMmaEncodingAttr::getWarpsPerCTA() const {
  return SmallVector<unsigned>(getWarpsPerCTA__());
}
SmallVector<unsigned> AMDWmmaEncodingAttr::getWarpOrder() const {
  return ::getWarpOrder(*this);
}
SmallVector<unsigned> MACAMmaEncodingAttr::getWarpOrder() const {
  return ::getWarpOrder(*this);
}
SmallVector<unsigned> AMDWmmaEncodingAttr::getThreadOrder() const {
  return ::getOrder(*this);
}
SmallVector<unsigned> MACAMmaEncodingAttr::getThreadOrder() const {
  return ::getOrder(*this);
}
...
SmallVector<unsigned> MACAMmaEncodingAttr::getThreadsPerWarp() const {
  if (getColMajor() == 0) {
    return {4, 16};
  } else {
    return {16, 4};
...
SmallVector<unsigned> MACAMmaEncodingAttr::getSizePerThread() const {
  auto elemsMnk = getElementsMNK();
  if (getColMajor() != 0) {
    return {elemsMnk[0], 4 * elemsMnk[1]};
  } else {
    return {4 * elemsMnk[0], elemsMnk[1]};
  }
}
...
SmallVector<unsigned>
MACAMmaEncodingAttr::getSizePerThreadForOperands(unsigned opIdx) const {
  auto elemsMnk = getElementsMNK();
  if (opIdx == 0) {
    return {elemsMnk[0], elemsMnk[2]};
  } else {
    return {elemsMnk[2], elemsMnk[1]};
  }
}
...
SmallVector<unsigned>
MACAMmaEncodingAttr::getShapePerCTATileForDotOperands(ArrayRef<int64_t> shape,
                                                      int opIdx) const {
  auto parentShapePerCTA = getShapePerCTATile(shape);
  auto rank = shape.size();
  assert(rank = 2);
  auto elemsMnk = getElementsMNK();
  if (opIdx == 0) {
    return {parentShapePerCTA[0], 4 * elemsMnk[2]};
  } else if (opIdx == 1) {
    return {4 * elemsMnk[2], parentShapePerCTA[1]};
  } else {
...
unsigned MACAMmaEncodingAttr::getTotalElemsPerThreadForOperands(
    ArrayRef<int64_t> shape, Type eltTy, int kWidth, int opIdx) const {

    int warpsPerCTAM = getWarpsPerCTA()[0];
    int warpsPerCTAN = getWarpsPerCTA()[1];

    auto elemsPerThread = getElementsMNK();
    if (opIdx == 0) {
      return getANumElemsPerThread(shape ,warpsPerCTAM, elemsPerThread);
    } else {
      return getBNumElemsPerThread(shape ,warpsPerCTAN, elemsPerThread);
    }
}
...
mlir::LogicalResult mlir::triton::gpu::SwizzleTensorOp::inferReturnTypes(
  ...
  if ((!srcTensorTy.getElementType().isIntOrIndex()) || isa<triton::PointerType>(srcTy))
    return mlir::failure();
...
calSubCtaNumAndSizePerThread(RankedTensorType srcTy, ArrayRef<int64_t> linearCtaIdx, ArrayRef<int64_t> linearElemIdx) {
  Attribute srcEncoding = srcTy.getEncoding();
  SmallVector<unsigned> retNumCtas, retSizePerThread;

  auto sizePerThread = mlir::triton::gpu::getSizePerThread(srcEncoding);
  auto shapePerCTA = mlir::triton::gpu::getShapePerCTATile(srcEncoding);
  SmallVector<unsigned> numReplica;
  for (int i=0; i<shapePerCTA.size(); i++) {numReplica.push_back(ceil<unsigned>(srcTy.getShape()[i], shapePerCTA[i]));}

  SmallVector<unsigned> order;
  int rank;
  if (isa<mlir::triton::gpu::MACAMmaEncodingAttr>(srcEncoding)) {
    auto mmaLayout = dyn_cast<mlir::triton::gpu::MACAMmaEncodingAttr>(srcEncoding);
    sizePerThread = SmallVector<unsigned>{mmaLayout.getElementsMNK()[0], mmaLayout.getElementsMNK()[1]};
    ...
    order = mlir::triton::gpu::getOrder(srcEncoding);
    rank = order.size();
  } else if (isa<mlir::triton::gpu::DotOperandEncodingAttr>(srcEncoding)) {
    auto dotLayout = dyn_cast<mlir::triton::gpu::DotOperandEncodingAttr>(srcEncoding);
    ...
    llvm::report_fatal_error("Unsupported layout for ExtractTensorOp Or InsertTensorOp");
  ...
  for (int i=0; i<linearCtaIdx.size(); i++) {
    auto multiCtaId = mlir::triton::getSubMultiDimIndex<unsigned>(linearCtaIdx[i], numReplica, order);
    for (int j=0; j<rank; j++) {
      ctaIdSet[j].insert(multiCtaId[j]);
    }
  }
  for (int i=0; i<linearElemIdx.size(); i++) {
    auto multiElemId = mlir::triton::getSubMultiDimIndex<unsigned>(linearElemIdx[i], sizePerThread, order);
    for (int j=0; j<rank; j++) {
...
mlir::LogicalResult mlir::triton::gpu::ExtractTensorOp::inferReturnTypes(
  ...
  if (isa<mlir::triton::gpu::MACAMmaEncodingAttr>(srcEncoding)) {
    auto threadsPerWarp = mlir::triton::gpu::getThreadsPerWarp(srcEncoding);
    auto warpsPerCTA = mlir::triton::gpu::getWarpsPerCTA(srcEncoding);
    auto mmaLayout = dyn_cast<mlir::triton::gpu::MACAMmaEncodingAttr>(srcEncoding);
    
    SmallVector<unsigned> newElemMNK = {retSizePerThread[0], retSizePerThread[1], mmaLayout.getElementsMNK()[2]};
    if (mmaLayout.getColMajor()) {
      retSizePerThread[1] *= 4;
    } else {
      retSizePerThread[0] *= 4;
    }
    for (int i=0; i<rank; i++) retShape[i] = retSizePerThread[i] * threadsPerWarp[i] * warpsPerCTA[i] * retNumCtas[i];
    auto retEncoding = mlir::triton::gpu::MACAMmaEncodingAttr::get(srcTy.getContext(),
                                      mmaLayout.getVersionMajor(), mmaLayout.getVersionMinor(), 
                                      warpsPerCTA, newElemMNK, mmaLayout.getColMajor(),
                                      mmaLayout.getIsATrans(), mmaLayout.getIsBTrans(),
                                      mmaLayout.getElementsStride());
    inferredReturnTypes.push_back(RankedTensorType::get(retShape, srcEltTy, retEncoding));

  } else if (isa<mlir::triton::gpu::BlockedEncodingAttr>(srcEncoding)) {
    auto threadsPerWarp = mlir::triton::gpu::getThreadsPerWarp(srcEncoding);
    auto warpsPerCTA = mlir::triton::gpu::getWarpsPerCTA(srcEncoding);
    for (int i=0; i<rank; i++) retShape[i] = retSizePerThread[i] * threadsPerWarp[i] * warpsPerCTA[i] * retNumCtas[i];
    auto retEncoding = mlir::triton::gpu::BlockedEncodingAttr::get(srcTy.getContext(),
                                          retSizePerThread, threadsPerWarp, warpsPerCTA,
                                          mlir::triton::gpu::getOrder(srcEncoding),
    ...
    auto dotLayout = dyn_cast<mlir::triton::gpu::DotOperandEncodingAttr>(srcEncoding);
    auto mmaLayout = dyn_cast<mlir::triton::gpu::MACAMmaEncodingAttr>(dotLayout.getParent());
    auto parentWarpsPerCTA = mlir::triton::gpu::getWarpsPerCTA(mmaLayout);
    auto opIdx = dotLayout.getOpIdx();

    SmallVector<unsigned> shapePerCTA, newElemMNK;
    if (opIdx == 0) {
      shapePerCTA = {16 * parentWarpsPerCTA[0] * retSizePerThread[0], 4 * retSizePerThread[1]};
      newElemMNK = {retSizePerThread[0], mmaLayout.getElementsMNK()[1], retSizePerThread[1]};
    } else if (opIdx == 1) {
      shapePerCTA = {4 * retSizePerThread[0], 16 * parentWarpsPerCTA[1] * retSizePerThread[1]};
      newElemMNK = {mmaLayout.getElementsMNK()[0], retSizePerThread[1], retSizePerThread[0]};
    }
    for (int i=0; i<rank; i++) retShape[i] = shapePerCTA[i] * retNumCtas[i];

    auto parentEncoding = mlir::triton::gpu::MACAMmaEncodingAttr::get(srcTy.getContext(),
                                      mmaLayout.getVersionMajor(), mmaLayout.getVersionMinor(), 
                                      parentWarpsPerCTA, newElemMNK, mmaLayout.getColMajor(),
                                      mmaLayout.getIsATrans(), mmaLayout.getIsBTrans(),
                                      mmaLayout.getElementsStride());
...
mlir::LogicalResult mlir::triton::gpu::InsertTensorOp::inferReturnTypes(
  ...
  auto linearCtaIdx = properties.as<Properties *>()->ctaIdx.asArrayRef();
  auto linearElemIdx = properties.as<Properties *>()->elemIdx.asArrayRef();
  auto res = calSubCtaNumAndSizePerThread(insertedTy, linearCtaIdx, linearElemIdx);
  auto retNumCtas = res.first;
  auto retSizePerThread = res.second;
  int rank = retNumCtas.size();
  ...
  if (isa<mlir::triton::gpu::MACAMmaEncodingAttr>(srcEncoding)) {
    ...
    for (int i=0; i<rank; i++) subShape[i] = retSizePerThread[i] * threadsPerWarp[i] * warpsPerCTA[i] * retNumCtas[i];
  } else if (isa<mlir::triton::gpu::BlockedEncodingAttr>(srcEncoding)) {
    auto threadsPerWarp = mlir::triton::gpu::getThreadsPerWarp(srcEncoding);
    auto warpsPerCTA = mlir::triton::gpu::getWarpsPerCTA(srcEncoding);
    for (int i=0; i<rank; i++) subShape[i] = retSizePerThread[i] * threadsPerWarp[i] * warpsPerCTA[i] * retNumCtas[i];    
    ...
    for (int i=0; i<rank; i++) subShape[i] = shapePerCTA[i] * retNumCtas[i];    
  }
